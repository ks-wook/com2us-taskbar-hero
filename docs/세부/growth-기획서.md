...
>
> 본 문서는 플레이어 캐릭터의 **성장 축 중 직업(클래스)·스킬·룬**을 서버 권위로 관리·검증하는 규칙을 다룬다. 저장 골격은 [세이브 데이터 기획서](save-data-기획서.md)(`game_player`·`player_character`·`player_growth`), 정적 정의는 [마스터 데이터 기획서](master-data-기획서.md)(`class_master`·`skill_master`·`rune_master`)를 참고한다. 펫(`growth_type=3`)·장비 강화는 본 문서 범위 밖이다(장비 강화는 [인벤토리/아이템/큐브 기획서](inventory-item-cube-기획서.md) 5.3).
...
- **범위 경계**:
  - **직업 선택**은 각 캐릭터 생성 시 확정된다([세이브 데이터 기획서](save-data-기획서.md) 5.3 `create-character`). 본 문서는 직업이 성장에 미치는 역할(기본 스탯·보유 스킬)을 다룬다. **전직은 지원하지 않는다.**
...
- **스킬**: 직업별로 보유하는 능력으로 **액티브/패시브**로 나뉜다(구분은 `skill_master`). **스킬 포인트를 소모**해 해당 캐릭터의 스킬 레벨(`player_growth.level`)을 올리며, 레벨별 효과는 `skill_master.effect_per_level`이 정의한다. 캐릭터는 자기 직업 소속 스킬만 올릴 수 있다. **액티브 스킬은 캐릭터당 최대 2개까지만 장착·사용**할 수 있고(패시브는 개수 제한 없이 상시 적용), **스킬 초기화(리셋)는 무료로 지원**한다(5.2).
...
**기능 요구사항**
- 성장 상태 조회는 별도 API를 두지 않고 [세이브 로드](save-data-기획서.md)(`POST /api/game/load`)가 `characters`(캐릭터별 직업·레벨·경험치)와 `growth`(스킬·룬 레벨) 스냅샷을 반환한다. 사용 가능한 스킬 포인트는 레벨과 보유 스킬 레벨에서 **파생 계산**한다(저장하지 않음). 본 문서는 **상태를 바꾸는 액션**만 전용 엔드포인트로 제공한다.
...
- **동시성/멱등성**: 단일 세션 정책([계정/로그인 기획서](account-login-기획서.md))으로 경합은 제한적이나, 대상 행(`(user_id, character_id, growth_type, code)`·`user_id`)에 잠금을 걸어 중복 레벨업·중복 차감을 막는다.

## 4. 데이터 모델

성장 저장의 핵심은 **어떤 스킬/룬이 몇 레벨인지**이며, 스킬 포인트는 저장하지 않고 캐릭터 레벨에서 파생한다. 액티브 스킬 장착(5.3)을 위해 `player_growth`에 `equipped`(0/1) 컬럼 1개를 둔다(스킬 행에만 의미). **3인 파티 구조상 캐릭터별 데이터는 캐릭터 단위 키(`character_id`)를 가진다** — [세이브 데이터 기획서](save-data-기획서.md)에 `player_character` 테이블 신설, `player_growth`(스킬)·`player_equipment`에 `character_id`, `player_growth`에 `equipped`(스킬 장착) 반영 완료. 룬(`growth_type=2`)은 계정 공용이라 `character_id=0`으로 저장한다.
...
| `player_growth`(`growth_type=1`, `code`=`skill_code`, `level`, `equipped`) | 스킬 레벨·액티브 장착 여부 | **캐릭터별**(+`character_id`) | `skill_master` |
| `player_growth`(`growth_type=2`, `code`=`rune_code`, `level`) | 룬 레벨 | **계정 공유** | `rune_master` |
| `player_currency`(`(user_id, currency_type)` PK, `amount`) | 룬 업그레이드 골드 차감 | 계정 공유 | `currency_master` |
...
- **스킬 레벨(`player_growth`, `growth_type=1`, `code`=`skill_code`, 캐릭터별)**: 0(미습득)부터 `skill_master.max_level`까지. 대상 캐릭터의 스킬 포인트로 1레벨씩 올린다. 행이 없으면 레벨 0으로 간주하고, 첫 레벨업 시 행을 생성한다. **사용 가능 포인트 = 해당 캐릭터 레벨 비례 총량 − 그 캐릭터가 이미 투자한 포인트(보유 스킬 레벨로부터 계산)** 로 서버가 산출하므로, 스킬 포인트 잔량을 따로 저장하지 않는다. **스킬 초기화(5.2)** 시 해당 캐릭터의 스킬 행을 모두 제거해 포인트를 전량 회수한다(무료, `equipped`도 함께 해제). 액티브 스킬은 `equipped`(0/1)로 장착 여부를 저장하며 캐릭터당 최대 2개가 `equipped=1`이다(장착 설정은 5.3).
- **룬 레벨(`player_growth`, `growth_type=2`, `code`=`rune_code`, 계정 공유)**: 0(미해금)부터 `rune_master.max_level`까지. 골드로 올리며, 선행 룬(`prereq_code`)이 **레벨 1 이상**이어야 해금(0→1) 가능하다. **룬 초기화는 지원하지 않는다.**

**공유 enum / DTO (TaskbarHero.Common)**
- `growth_type`(1:스킬 2:룬 3:펫)은 [세이브 데이터 기획서](save-data-기획서.md)·[마스터 데이터 기획서](master-data-기획서.md)와 동일한 값으로 `TaskbarHero.Common`에 enum 고정한다(값 변경 금지). 펫(3)은 본 문서 범위 밖이나 enum 값은 공유한다.
...
- 트랜잭션: 해당 캐릭터의 `player_growth`(`growth_type=1`) 행을 모두 삭제. 재화 변동 없음.
...
- 트랜잭션: 해당 캐릭터 스킬 행의 `player_growth.equipped`를 요청 목록에 맞춰 갱신(목록에 있으면 1, 없으면 0).
...
트랜잭션(BEGIN, (user_id, characterId) 잠금)
  ...
  3) cur = player_growth[user_id, characterId, 1, skillCode].level or 0
  ...
  5) 반영: player_growth.level = cur + 1 (없으면 INSERT)      # 사용 포인트는 스킬 레벨에 내재, 별도 차감 없음
COMMIT → { characterId, skillCode, level, cost, skillPoint = total - (spent + need) }
```

**스킬 초기화**(5.2)는 위 잠금 아래에서 대상 캐릭터의 `player_growth`(`growth_type=1`) 행을 전부 삭제하고 커밋한다(비용·RNG 없음). 삭제 후 사용 가능 포인트는 그 캐릭터 레벨 비례 총량 전액으로 복원된다.
...
트랜잭션(BEGIN, user_id 잠금)
  ...
  3) cur = player_growth[user_id, 0, 2, runeCode].level or 0   # 룬은 계정 공용(character_id=0)
     if cur >= rune.max_level: RuneMaxLevel(5011)
  4) cost = 서버 산출(rune.cost, 현재 레벨 cur에 비례)
     if gold < cost: InsufficientCurrency(4005)
  5) 반영: gold -= cost; player_growth.level = cur + 1 (없으면 INSERT, character_id=0)
...
- **동시 중복 요청**: 같은 `(user_id, character_id, growth_type, code)`에 대한 레벨업이 겹치면 행 잠금으로 직렬화한다.
...
- **액티브 스킬 장착 (확정 · 반영 완료)**: 액티브 스킬은 **캐릭터당 최대 2개**만 장착·사용(패시브는 제한 없이 상시 적용). 장착은 API 5.3(`skill/equip`)으로 설정하고 `player_growth.equipped`(0/1)에 저장한다. 남은 미결: 액티브 스킬의 **전투 시 발동 순서·쿨다운 등 사용 규칙**은 전투 도메인(4.6)과 함께 정의.

## 9. 참고

- [서버 시스템 전체 개요](../공통/서버-시스템-전체-개요.md) — 도메인 4.5(성장), 4.6(전투=경험치 획득)
- [세이브 데이터 기획서](save-data-기획서.md) — `game_player`·`player_character`·`player_growth` 저장 골격, 로드 스냅샷
