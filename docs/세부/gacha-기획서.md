...
| `weight_up` | int | 소프트 전용 — 발동 이후 1회당 그 등급 가중치에 더할 값 |
| `weight_up_max` | int | 소프트 전용 — 가산 상한(누적 가산치의 최대값, 0 = 무제한) |
...
- **소프트 천장을 확률(%)이 아니라 가중치 가산으로 정의한다(확정).** 등급 선택이 이미 가중치 방식이라, 가중치만 더하면 나머지 등급의 확률이 **자동으로 비례 감소**한다. 참고 구현처럼 "타겟 확률을 올린 뒤 총합 100%가 되도록 나머지를 재조정"하는 별도 정규화 단계가 필요 없고, 반올림 오차로 총합이 어긋날 여지도 없다.
- **소프트의 상한은 하드가 대신한다.** 소프트 구간(70~89회차)이 끝나면 90회차에서 하드가 100%로 끊으므로 `weight_up_max`를 쓸 일이 없다(`0`=무제한으로 둔다). 하드 없이 소프트만 두는 배너에서만 상한이 의미를 갖는다.
...
|---|---|---|---|
| 5 | 1 (소프트) | **70** | **70회차부터** 회차마다 5등급 가중치를 `weight_up`씩 가산해 확률이 오른다 |
...
- `weight_up`은 **5등급 기본 가중치와 같은 값**이다(확정값 `100`) — 발동 후 k회차째에 5등급 가중치가 `기본 × (1 + k)`가 되는 선형 상승이다.
...
| `60002` | 성검 엑스칼리버 픽업 | **한정** | 2026-08-03 ~ 08-17 (14일) | 60,000 / 540,000 | `31151` | **성검 엑스칼리버 1종** |
...
|---|---|---|---|
| 소프트 | 1 | `pullNo >= threshold`부터 그 등급 가중치를 `(pullNo - threshold + 1) × weight_up`만큼 가산. 확률이 점점 오르지만 확정은 아니다 | `threshold=70` → **70~89회차 상승** |
