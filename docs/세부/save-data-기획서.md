...
- **쓰기는 MySQL 직접, 읽기 캐시는 예외(확정)**: 세이브 데이터의 정본은 항상 MySQL이며 **쓰기에는 Redis 계층을 두지 않는다** — 인벤토리 칸 배치(`player_item.slot`)처럼 빈도가 높아 보이는 쓰기도 지연 반영하지 않는다(판단 근거: [인벤토리/아이템/큐브 기획서](inventory-item-cube-기획서.md) 5.5). 유일한 예외는 **가방 페이지 조회(5.2)의 읽기 캐시**로, write-through라 정본이 옮겨가지 않는다(같은 문서 6.5). (인증 토큰 검증용 Redis는 별개 용도이며 세이브 저장과 무관하다.)
...
> 이 조회는 **Redis write-through 캐시**로 받는다([인벤토리/아이템/큐브 기획서](inventory-item-cube-기획서.md) 6.5). 캐시는 파생 데이터이고 정본은 MySQL이며, 미스·장애 시 MySQL 직접 경로로 폴백하므로 아래 응답 규약은 캐시 사용 여부와 무관하게 동일하다.
