...
- **읽기·쓰기 모두 MySQL 직접(확정)**: 세이브 데이터의 정본은 항상 MySQL이며 **Redis 계층을 두지 않는다** — 인벤토리 칸 배치(`player_item.slot`)처럼 빈도가 높아 보이는 쓰기도 지연 반영하지 않고(판단 근거: [인벤토리/아이템/큐브 기획서](inventory-item-cube-기획서.md) 5.5), **가방 페이지 조회(5.2)의 읽기도 캐시하지 않는다**(같은 문서 6.5 — 계정별 개인 데이터이고 인덱스 keyset 질의라 캐시 이득이 없다). (인증 토큰 검증용 Redis는 별개 용도이며 세이브 저장과 무관하다.)
...
- **PK/유니크**:
  ...
  - `player_buff`: `(user_id, buff_type)` 복합 PK — 계정당 버프 **종류별 1행**이며, 이 PK 행 잠금이 같은 종류의 중복 사용 요청을 직렬화한다(별도 분산 락 없음). 종류가 다른 버프(경험치·골드)는 별도 행이라 동시 활성된다. 활성 판정은 `expires_at > now`이며(배율 적용도 같은 기준), 만료 행은 모든 조회에서 걸러지므로 정리 배치가 여유를 두고 삭제한다([소모품/버프 기획서](consumable-buff-기획서.md) 4.2·6.4). 정리 배치용 `expires_at` 보조 인덱스를 둔다.
...
- **아이템·재화 통합(`row_type`)**: `player_item`은 `row_type`(1:아이템 2:재화)으로 아이템과 재화(골드 등)를 **한 테이블에** 담는다. `item_code`는 **모든 행이 `item_master.item_code`를 참조**하며(재화는 `item_master`의 `item_type=3` 항목, 골드=`item_code` 1 — 별도 `currency_master` 없음), `quantity`가 수량/재화 금액(재화가 커 `bigint`)이다. **재화 행은 계정에 재화 종류당 1행**이어야 하므로 `(user_id, row_type=2, item_code)` 유일성을 **서버가 보장**한다(MySQL 부분 유니크 인덱스 미지원. 아이템 행은 스택 분할로 `(user_id, item_code)`가 중복될 수 있어 전역 유니크를 걸 수 없다). 보장 방법은 **세이브를 만들 때 재화 행을 미리 만들어 두고(5.3), 그 뒤로는 어떤 경로도 재화 행을 새로 만들거나 지우지 않는 것**이다 — 지급이 "행이 없으면 만든다"를 판단하면, 잔액 0인 계정에 지급 두 건이 동시에 들어올 때 양쪽이 모두 '행 없음'을 보고 각자 만들어 재화 행이 둘로 갈라진다. 재화 행은 `slot`/`enhance_level`을 쓰지 않고 장착 대상도 아니며(`player_item_equipped`에 행이 생기지 않음) **인벤토리 용량 집계에서 제외**한다.
...
- **가챠 천장 진행도는 코어에 넣지 않는다.** 가챠 화면을 열 때만 필요한 값이고, 그때 호출하는 배너 조회(`POST /api/game/gacha/banners`)가 열려 있는 배너와 함께 내려준다([가챠 기획서](gacha-기획서.md) 5.1). 가방을 창고 UI에서 조회하는 것과 같은 판단이다 — 접속 즉시 필요하지 않은 데이터는 코어 스냅샷을 키우지 않는다.
...
가방 아이템을 **`slot` 커서 기준 keyset 페이징**으로 조회한다. 접속 직후에는 호출하지 않고, **창고/인벤토리 UI를 열 때마다 호출한다** — 자동 전투로 전리품이 계속 적재되므로 클라이언트 로컬 캐시를 신뢰하지 않고 서버 상태를 다시 받는다.

> 이 조회에는 **캐시를 두지 않는다** — `cursor`·`limit`을 그대로 질의로 넘겨 `(user_id, slot)` 유니크 인덱스로 **필요한 구간만** MySQL에서 읽는다(조인 없음·filesort 없음). 판단 근거는 [인벤토리/아이템/큐브 기획서](inventory-item-cube-기획서.md) 6.5.
...
- **근거**: 단일 세션 정책([계정/로그인 기획서](account-login-기획서.md))상 한 계정의 클라이언트는 하나다. 거래 체결·만료 반송은 아이템이 **메일**로 가고, 메일 수령·큐브·장착·이동·거래 등록은 전부 유저 행동이라 페이징 요청과 순차다. 배치(`MailGcBatchScheduler`·`TradeExpireBatchScheduler`)는 가방을 건드리지 않는다. 남는 경합은 **방치 전투 전리품**뿐인데, 그것을 발생시키는 주체도 같은 클라이언트라 자기가 바꿨다는 사실을 이미 안다.
...
> **기본 무기 지급·장착**: 생성되는 캐릭터는 **그 직업의 가장 등급 낮은 무기를 장착한 상태로 시작**한다(맨손으로 전투에 나가지 않게 하기 위함). 최초 생성·추가 생성 모두 같으며, **캐릭터 삽입과 같은 트랜잭션**에서 `player_item` 1행(수량 1·강화 0) + `player_item_equipped` 1행(무기 슬롯 `1`)을 함께 적재하므로 무기 없는 캐릭터가 남지 않는다.
> - **어떤 무기인가**: 별도 마스터 테이블을 두지 않고 `item_master`에서 파생한다 — 후보는 `item_type=1`(장비) · `equip_slot=1`(무기) · `class_req=요청 직업` · `level_req ≤ 1`(레벨 1이 장착 가능)인 아이템이며, 그중 **등급이 가장 낮고 동급이면 `item_code`가 작은** 것을 고른다. 현재 값 기준 기사 `31111`(낡은 검) · 레인저 `31211`(낡은 활) · 마법사 `31311`(나무 지팡이) · 슬레이어 `31411`(녹슨 도끼)이다. 무기를 추가·수정해도 이 규칙이 그대로 따라가므로 코드는 바뀌지 않는다.
> - **가방 칸을 쓰지 않는다**: 장착 중인 장비는 `player_item.slot = NULL`이라는 규칙([인벤토리/아이템/큐브 기획서](inventory-item-cube-기획서.md) 5.1 장착)을 그대로 따르므로, 가방이 가득 찬 계정에서도 지급이 실패하지 않으며 `inventoryTotal`도 늘지 않는다. 플레이어가 이 무기를 해제하면 그때 가방 빈 칸으로 들어온다(일반 장비와 동일).
> - **응답에 담지 않는다**: 생성 직후 클라이언트가 5.1 `load`를 재호출해 `equipped`로 받으므로 `create-character` 응답 필드는 늘리지 않는다.
> - 마스터에 그 직업의 무기가 없으면(정의 결함) 지급을 건너뛰고 캐릭터만 생성하며, 서버는 Error 로그를 남긴다.

> **기본 액티브 스킬 습득·장착**: 생성되는 캐릭터는 **그 직업의 첫 번째 액티브 스킬을 레벨 1로 습득하고 장착까지 마친 상태로 시작**한다(기본 무기와 같은 이유 — 스킬 하나 없이 평타만으로 전투에 나가지 않게 하기 위함). 최초 생성·추가 생성 모두 같으며, **캐릭터 삽입과 같은 트랜잭션**에서 `player_skill` 1행(`level=1`·`equipped=1`)을 함께 적재하므로 스킬 없는 캐릭터가 남지 않는다.
> - **어떤 스킬인가**: 기본 무기와 마찬가지로 별도 마스터 테이블을 두지 않고 `skill_master`에서 파생한다 — 후보는 그 직업의 **액티브 스킬(`skill_type=1`)** 이며, 그중 **`skill_code`가 가장 작은** 것을 고른다. 코드 규약(직업별 액티브 `x01~x03`, [마스터 데이터 값](master-data/master-data-값.md) 4장)상 이는 각 직업의 첫 액티브다 — 기사 `101`(방패 돌진) · 레인저 `201`(정조준 사격) · 마법사 `301`(파이어볼) · 슬레이어 `401`(내려찍기). 스킬을 추가·수정해도 이 규칙이 그대로 따라간다.
> - **장착까지 하는 이유**: 액티브 스킬은 `equipped=1`이어야 전투에서 쓰인다([성장 기획서](growth-기획서.md) 5.3). 습득만 시키면 플레이어가 장착 UI를 거쳐야 첫 스킬을 쓸 수 있으므로, 장착 한도 2개 중 첫 칸을 채워 둔다(새 캐릭터라 기존 장착과 충돌하지 않는다).
> - **스킬 포인트 1을 미리 투자한 상태다(무상 지급이 아니다)**: 사용 포인트를 저장하지 않고 **그 캐릭터의 스킬 레벨 합**으로 파생하는 회계를 그대로 따르므로([성장 기획서](growth-기획서.md) 4장), 레벨 1 캐릭터의 잔여 포인트는 1 → **0**이 된다. 예외 규칙을 만들지 않아 **스킬 초기화(성장 기획서 5.2) 시 그 1포인트도 정상 회수**되며, 이때 기본 스킬은 레벨 0·미장착이 되어 미습득 상태로 돌아간다(플레이어가 다시 찍을 수 있다).
> - **응답에 담지 않는다**: 기본 무기와 같은 이유로 생성 직후 `load`의 `skills`로 받는다.
> - 마스터에 그 직업의 액티브 스킬이 없으면(정의 결함) 습득을 건너뛰고 캐릭터만 생성하며, 서버는 Error 로그를 남긴다.

> **재화 행 미리 만들기**: 계정 세이브가 처음 만들어질 때(= 최초 캐릭터 생성) **골드 재화 행을 잔액 0으로 함께 넣는다**(`player_item`, `row_type=2`, `item_code=1`). 시작 잔액이 0인 것은 신규 지원금을 아래 환영 메일로 주기 때문이다.
> - **왜 미리 만드나**: 이렇게 해 두면 골드를 주는 모든 경로(스테이지 보상·메일 수령·방치 보상·큐브 분해)가 "재화 행이 없으면 만든다"를 판단할 필요가 없어진다. 그 판단이 남아 있으면 잔액 0인 계정에 지급 두 건이 동시에 들어올 때 **양쪽이 모두 '행 없음'을 보고 각자 만들어 재화 행이 둘로 갈라지고**, 그 뒤로는 조회가 한 행만 읽어 나머지 잔액이 보이지 않게 된다.
> - **행은 지우지 않는다**: 잔액이 0이 되어도 행을 남긴다. 지우면 위의 경합이 되살아난다.
> - 재화 행은 `slot`이 NULL이라 가방 용량·페이징에 잡히지 않으므로, 미리 만들어도 플레이어에게 보이는 변화는 없다.
