...
> - **가방 칸을 쓰지 않는다**: 장착 중인 장비는 `player_item.slot = NULL`이라는 규칙(5.6 장착)을 그대로 따르므로, 가방이 가득 찬 계정에서도 지급이 실패하지 않으며 `inventoryTotal`도 늘지 않는다. 플레이어가 이 무기를 해제하면 그때 가방 빈 칸으로 들어온다(일반 장비와 동일).
