...
- [7. 성능 설계](#7-성능-설계)
  ...
  - [7.4 Redis 락(구매·취소·만료·등록)](#74-redis-락구매취소만료등록)
...
- **Redis(상시 사용)**: 새로 요구되는 **영속 테이블은 `trade_listing` 하나뿐**이다. Redis에는 목록 캐시 키 2종([7.3](#73-redis-목록-캐시))과 락 키 2종(등록 단위·판매자 단위, [7.4](#74-redis-락구매취소만료등록))을 두며, 모두 언제든 MySQL에서 다시 만들 수 있거나 TTL로 사라지는 **파생 데이터**다(영속 데이터를 Redis에만 두지 않는다).
...
COMMIT → 목록 캐시에서 해당 등록 제거(7.3) → 락 해제(자기 락일 때만) → { listingId, gained, cost, balance, mailId }
```
...
- **취소(5.4)**: Redis 등록 단위 락 획득([7.4](#74-redis-락구매취소만료등록), 구매와 같은 키) → 본인·판매중 확인 → `player_item`에 아이템 복원 → 조건부 갱신으로 `status=3`(수동 취소) → 캐시에서 제거 → 락 해제. 인벤토리 용량 초과면 `InventoryFull(4002)`.
...
### 7.4 Redis 락(구매·취소·만료·등록)

**목적**: 같은 대상에 몰린 요청을 **MySQL에 도달하기 전에 한 개로 줄인다.** 어차피 1명만 성공하는 경합에서, 나머지 요청이 트랜잭션·커넥션을 잡고 롤백하는 낭비를 없애는 것이 락의 역할이다.

키는 두 종류다 — 상태를 닫는 경로(구매·취소·만료)는 **등록 단위**, 판매 등록은 **판매자 단위**로 잡는다.

| 항목 | 값(잠정) |
|---|---|
| 키 | `trade:lock:listing:{listingId}`(구매·취소·만료) · `trade:lock:seller:{userId}`(판매 등록) |
...
- **락을 쓰는 경로**: 구매 · 판매 취소 · 만료 배치는 **등록 단위 키**를 공유해, 같은 등록 행을 닫는 모든 경로가 하나의 직렬화 지점을 지나게 한다. **목록 조회는 락을 쓰지 않는다**(읽기이며 정합성 정본은 MySQL이다).
- **판매 등록은 판매자 단위 키를 쓴다(확정)**: 등록 시점에는 아직 `listing_id`가 없고, 경합 대상이 **그 계정의 동시 등록 수**다. 한도 검사(`COUNT`)와 삽입 사이에 같은 계정의 다른 요청이 끼어들면 **한도(10건)를 넘길 수 있다**(9건에서 두 요청이 동시에 통과 → 11건). 판매자 키로 잡으면 그 계정의 등록이 직렬화되어 이 경합이 사라진다.
  - **단, 보증이 아니라 완화다.** Redis 장애 시에는 락 없이 진행하므로(축소 운전, [7.5](#75-redis-장애와-캐시-불일치)) 경합이 다시 가능해진다. 초과 결과가 무해(한도가 스팸 방지 목적이고 1~2건 초과)하고 그 대가로 가용성을 지키는 선택이다. **DB 차원의 보증이 필요해지면** `(seller_user_id, slot_no)` 유니크 제약으로 슬롯을 배정하는 방식으로 바꿀 수 있다(테이블 추가 없이 컬럼 + 유니크 인덱스).
...
- `TradeBusy(7008)`는 [7.4](#74-redis-구매-락)의 Redis 구매 락 획득에 재시도까지 실패했을 때 쓴다. HTTP **409 Conflict**로 응답하며, 클라이언트는 잠시 후 재시도하면 성공할 수 있다.
