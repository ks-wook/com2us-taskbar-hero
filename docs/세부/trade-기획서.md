...
- **가용성**: Redis를 목록 캐시로만 쓰되 **단일 장애점으로 두지 않는다**. 장애 시 목록 조회는 MySQL 폴백으로 계속 동작한다(성능 저하는 허용, 기능 중단은 불허 — [7.5](#75-redis-장애와-캐시-불일치)). **등록·구매·취소·만료는 Redis를 쓰지 않으므로 장애와 무관하다**(정합성이 MySQL 안에서 끝난다).
...
- **Redis(목록 캐시 전용)**: 새로 요구되는 **영속 테이블은 `trade_listing` 하나뿐**이다. Redis에는 목록 캐시 키 2종([7.3](#73-redis-목록-캐시))만 두며, 언제든 MySQL에서 다시 만들 수 있거나 TTL로 사라지는 **파생 데이터**다(영속 데이터를 Redis에만 두지 않는다). **락 키는 두지 않는다**([7.4](#74-동시성--애플리케이션-락을-두지-않는다)).
...
2. **Redis는 목록 캐시로 상시 사용한다**([7.3](#73-redis-목록-캐시)). 등록 수가 적어도 켠 상태로 운영해 코드 경로를 하나로 유지한다. **애플리케이션 락은 어느 경로에도 두지 않는다**([7.4](#74-동시성--애플리케이션-락을-두지-않는다)).
...
| 판매 등록 | **Redis와 무관하다** — 한도 검사·에스크로·등록 생성이 모두 MySQL 트랜잭션 안에서 끝난다([7.4](#74-동시성--애플리케이션-락을-두지-않는다)). 커밋 후 캐시 추가만 실패할 수 있고, 그 경우 다음 조회가 MySQL에서 재적재한다 |
...
로그는 [로깅 규칙](../공통/로깅-규칙.md)을 따르고, 지표는 **목록 캐시 히트율 · MySQL 폴백 횟수 · `TradeAlreadyClosed` 발생률**(구매 경합 강도) 셋을 본다.
...
>
> **확정 사항(성능·아키텍처)**: **Redis는 목록 캐시로만 쓴다**(데이터량과 무관하게 상시 사용) · 다만 **단일 장애점으로 두지 않는다**(장애 시 MySQL 폴백) · **모든 직렬화는 MySQL 행 잠금으로 처리하고 애플리케이션 락을 두지 않는다**(등록·구매·취소·만료 전부) · 정합성의 정본은 **MySQL 트랜잭션**이고 Redis에는 영속 데이터를 두지 않는다 · 캐시 갱신은 **커밋 이후**에만 수행 · 정렬 축은 **가격 오름차순 1축만** 유지한다 · 그 밖의 최적화는 [7.7](#77-하지-않는-것)에 따라 도입하지 않는다.
...
- [계정 / 로그인 기획서](account-login-기획서.md) — Redis 키 네이밍(`auth:token:{userId}`)·CloudStructures 사용 선례. 토큰 캐시는 없으면 인증 자체가 불가한 **필수 의존**이고, 거래소 Redis(목록 캐시)는 상시 사용하되 장애 시 MySQL 폴백으로 축소 운전이 가능하다([7.5](#75-redis-장애와-캐시-불일치))
