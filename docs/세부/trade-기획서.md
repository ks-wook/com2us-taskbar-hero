...
**Redis를 쓰는 곳과의 경계** — GameServer의 Redis 용도는 **인증 토큰 검증**과 **보스러시 랭킹 캐시**뿐이다(배치 분산 락은 BatchServer 분리로 없앴고, 남은 락은 랭킹 캐시 재적재가 인스턴스마다 중복으로 돌지 않게 하는 것 하나다). 거래소·인벤토리 등 게임 데이터 조회는 전부 MySQL 직접 경로다([인벤토리 기획서](inventory-item-cube-기획서.md) 6.5도 같은 판단).
