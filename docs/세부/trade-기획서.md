...
- 판매 대금·반송 아이템 **메일의 만료는 7일**이다(구매 아이템 메일만 무기한).
...
- 구매 아이템·판매 대금 모두 계정에 **즉시 반영하지 않고** 메일로 발급한다(우편함 수령 시 반영, [메일 기획서](mail-기획서.md)). **수수료 20%** 분 골드는 **경제에서 소멸**(sink)한다. 대금 메일은 만료 **7일**, 구매 아이템 메일은 **무기한**이다.
...
**캐시를 쓸 수 있는 조건** — 캐시는 전 사용자가 공유하는 **뷰어 무관** 데이터라, 판매자별로 걸러낸 결과는 담을 수 없다. 따라서 캐시로 응답하는 경우는 **판매중 등록이 하나도 없는 뷰어의 제외 모드 조회**뿐이다(이때 전체 목록 = 제외 후 목록이라 정확하다). 본인 등록이 있는 뷰어와 `mine=true` 조회는 MySQL 쿼리로 응답하고 그 결과를 캐시에 적재하지도 않는다. 뷰어의 판매중 등록 수는 `idx_trade_seller`만 타는 가벼운 조회로 확인한다.
