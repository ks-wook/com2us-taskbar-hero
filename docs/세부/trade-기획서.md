...
- [7. 성능 설계](#7-성능-설계)
  ...
  - [7.4 Redis 락(판매 등록 전용)](#74-redis-락판매-등록-전용)
...
- **가용성**: Redis를 상시 사용하되 **단일 장애점으로 두지 않는다**. 장애 시 목록 조회는 MySQL 폴백, 판매 등록은 락 없이 진행한다(성능 저하·한도 완화는 허용, 기능 중단은 불허 — [7.5](#75-redis-장애와-캐시-불일치)). **구매·취소·만료는 애초에 Redis를 쓰지 않으므로 장애와 무관하다**(정합성이 MySQL 안에서 끝난다).
...
- **Redis(상시 사용)**: 새로 요구되는 **영속 테이블은 `trade_listing` 하나뿐**이다. Redis에는 목록 캐시 키 2종([7.3](#73-redis-목록-캐시))과 **판매자 단위 락 키 1종**([7.4](#74-redis-락판매-등록-전용))을 두며, 모두 언제든 MySQL에서 다시 만들 수 있거나 TTL로 사라지는 **파생 데이터**다(영속 데이터를 Redis에만 두지 않는다).
...
- 오류: `ItemNotFound(4001)`(인벤토리에 없음), `ItemEquipped(4007)`(장착 중), `TradeNotSellable(7002)`(`sellable=0`), `TradePriceOutOfRange(7006)`(기준가 ±20% 범위 밖), `TradeListingLimitExceeded(7007)`(동시 등록 10개 초과), `TradeBusy(7008)`(같은 계정의 다른 등록 요청이 처리 중 — 판매자 락 경합, [7.4](#74-redis-락판매-등록-전용)), `InvalidSaveData(2002)`(형식 오류 등).
...
COMMIT → 목록 캐시에서 해당 등록 제거(7.3) → { listingId, gained, cost, balance, mailId }
```

- **선점(4번)을 재화 이동보다 앞에 둔다.** 상태 전이를 먼저 확정하면, 나머지 단계는 이 트랜잭션이 유일한 승자임이 보장된 상태에서 진행된다(근거 [7.2](#72-mysql-색인과-조건부-갱신)).
- **구매 경로에 Redis 락은 없다.** 경합 대상이 `trade_listing` 행 하나이고, 그 행을 조건부 갱신할 때 MySQL이 거는 **행 잠금이 이미 요청을 줄 세운다**. 앞단에 락을 하나 더 두면 직렬화 지점이 둘로 늘어 TTL 만료·Redis 장애 같은 경계 상황만 추가된다([7.4](#74-redis-락판매-등록-전용)).
...
2. **Redis는 목록 캐시로 상시 사용한다**([7.3](#73-redis-목록-캐시)). 등록 수가 적어도 켠 상태로 운영해 코드 경로를 하나로 유지한다. 락은 **판매 등록의 한도 검사에만** 쓴다([7.4](#74-redis-락판매-등록-전용)).
...
- **애플리케이션 락(Redis 등)을 앞단에 두지 않는다.** 경합 대상이 등록 행 하나뿐이라 행 잠금이 곧 직렬화이고, 락을 더 얹으면 정합성은 그대로인데 TTL 만료·락 해제 실패·Redis 장애라는 경계만 늘어난다([7.4](#74-redis-락판매-등록-전용)).
...
| 키 | 자료구조 | 내용 | 용도 |
|---|---|---|---|
| `trade:index:{itemCode}` | Sorted Set | score = 가격, member = `listingId`. **TTL 24시간**(적재 시점 기준 절대 만료) | 가격순 목록(`itemCode=0`이 전체 목록). 페이징은 서버 메모리에서 |
| `trade:listing:{listingId}` | String(JSON) | 등록 스냅샷(아이템·강화·수량·가격·판매자·등록 시각). TTL 3일 | 목록 응답 조립 + **결손이 곧 오염 감지 신호** |

**조회 경로**: Sorted Set에서 해당 `itemCode`의 `listingId` **전량**을 꺼내고, 그 스냅샷들을 **한 번에(왕복 1회분으로 묶어)** 읽어 응답을 만든다. MySQL을 건드리지 않는다. 정렬 축이 가격 하나뿐이므로(정렬 옵션 없음, 9장 확정 사항) Sorted Set 하나로 정렬이 해결된다.

**뷰어별 필터는 메모리에서 적용한다** — 캐시는 전 사용자가 공유하는 **뷰어 무관** 데이터지만, 색인이 **항상 완전 집합**이고(아래 「부분 색인 금지」) 스냅샷에 `sellerUserId`가 들어 있으므로, 캐시에서 전량을 읽어 **본인 등록 제외(`mine=false`) 또는 본인 등록만(`mine=true`) 필터와 페이징을 서버 메모리에서** 적용해도 결과가 정확하다. 따라서 **캐시가 적재돼 있으면 두 모드 모두 MySQL을 타지 않는다**(뷰어의 등록 보유 여부를 확인하는 별도 조회도 필요 없다).

**미적재면 무조건 전량을 읽어 캐시를 채운다(확정)** — 캐시 미스는 뷰어 필터·페이징을 걸지 않은 **전량 조회 1회**로 처리하고, 그 결과로 캐시를 채운 뒤 메모리에서 필터·페이징한다. 등록 규모를 미리 확인하는 건수 조회도, 대형 집합용 별도 경로도 두지 않는다 — 본 프로젝트는 이용자 수가 많지 않아 **판매중 등록 전량이 페이지 상한(100건)을 넘지 않는 규모**를 전제한다.

**전량 조회에 상한을 걸지 않는 이유** — 상한으로 잘라 읽으면 **부분 집합을 완전 집합처럼 캐시**해 이후 조회가 나머지 등록을 영구히 가린다(아래 「부분 색인 금지」와 같은 사고). 그래서 캐시 적재용 조회는 `LIMIT` 없이 전량을 읽는다. 다만 전제가 깨져 전량이 상한을 넘으면 응답은 계속 정확하되 메모리·Redis 사용량이 커지므로, 그 상황을 **Warning 로그로 남겨** 페이징 방식을 재검토할 신호로 쓴다(조용히 잘라내지 않는다).

**메모리 필터가 정확한 조건** — 필터 후 페이징이 정확한 것은 **완전 집합을 들고 있을 때만**이다. 부분 집합에 적용하면 `OFFSET`이 필터 전 기준이 되어 페이지 건수가 들쭉날쭉해지고 항목이 누락·중복된다. 위 두 규칙(전량 조회·부분 색인 금지)이 이 조건을 보장한다.

**부분 색인 금지** — 색인이 없는 상태에서 등록 1건만 넣으면 "원소 1개짜리 완전한 색인"처럼 보여, 이후 조회가 캐시 적중으로 판단하고 MySQL의 나머지 등록을 가린다. 그래서 등록 시에는 **이미 존재하는 색인에만** 추가하고, 적재(lazy fill)는 **해당 `itemCode`의 전체 집합을 담은 조회**(첫 페이지 + `hasMore=false`)일 때만 수행한다.

**갱신 규칙 — 세 줄만 지키면 된다.**

1. **MySQL 커밋이 끝난 뒤에** 캐시를 고친다. 커밋 전에 고치면 롤백 시 존재하지 않는 등록이 목록에 남는다.
2. 등록이면 색인·스냅샷을 **추가**하고, 구매·취소·만료면 **제거**한다.
3. 제거는 **스냅샷 → 색인** 순서로 한다(아래 「정리 실패와 자기 치유」).

**채우는 방법**: 기동 시 미리 적재하지 않는다. 해당 `itemCode`의 색인이 비어 있으면 그때 MySQL에서 **뷰어 필터·페이징 없이 전량**을 읽어 캐시를 채우고, 그 결과로 응답을 만든다(lazy). 이때 색인은 **덮어쓰기가 아니라 재구축**한다 — 기존 색인 키를 지우고 전량을 다시 넣는다. 스냅샷 TTL은 3일, **색인 TTL은 24시간**이며 색인 TTL은 **적재 시점 기준 절대 만료**로 조회·등록으로 갱신하지 않는다(갱신하면 인기 아이템의 색인이 영구히 만료되지 않아 아래 안전망이 무력화된다).

**정리 실패와 자기 치유** — 구매·취소·만료 후 캐시 제거가 실패해도 **거래는 되돌리지 않는다.** 그 시점에 MySQL 커밋은 이미 끝났으므로 되돌리려면 보상 트랜잭션(등록 상태 복원·골드 환불·발급 메일 삭제)이 필요한데, 그 보상도 실패할 수 있어 표시용 파생 데이터의 정리 실패를 실제 재화 이동의 되돌림으로 승격시키는 셈이 된다. 또 제거 실패의 주 원인은 Redis 장애이므로, 실패를 거래 실패로 처리하면 **파생 캐시가 거래소의 단일 장애점**이 되어 [7.5](#75-redis-장애와-캐시-불일치)의 가용성 원칙과 어긋난다. 캐시에 팔린 등록이 남아도 구매 시 MySQL 조건부 갱신이 `TradeAlreadyClosed(7005)`로 막으므로 **이중 판매는 발생하지 않고, 피해는 목록에 잠시 유령 항목이 보이는 표시 문제뿐**이다.

복구는 **실패한 그 순간이 아니라 이후 조회 시점**에 맡긴다(장애 중에는 어차피 아무것도 고칠 수 없다). 실패 유형별로 다음이 구제한다:

| 실패 유형 | 남는 상태 | 구제 장치 |
|---|---|---|
| 부분 실패(스냅샷은 지워졌고 색인 제거에서 끊김) | 색인에 스냅샷 없는 고아 id | 조회가 스냅샷 결손을 **감지**(캐시 미스로 처리) → 전량 재적재 시 **색인 재구축**으로 고아 제거 |
| 전부 실패(첫 호출부터 Redis 불통) | 색인·스냅샷 모두 잔존 = 유령 항목 | 감지 불가 → **색인 TTL(24시간)** 이 유일한 안전망(불일치 지속 시간의 상한) |

그래서 **제거는 스냅샷을 먼저** 지운다. 반대 순서면 부분 실패 시 스냅샷이 남아 유령 항목이 계속 조회되고, 감지 수단이 사라진다. 또 재적재가 색인을 **재구축**해야 한다 — 원소를 더하기만 하면 고아 id가 계속 남아 그 `itemCode`가 영구히 캐시 미스가 된다.
...
### 7.4 Redis 락(판매 등록 전용)

**Redis 락은 판매 등록 한 곳에만 쓴다.** 구매·판매 취소·만료 배치는 락 없이 동작하며, 직렬화는 MySQL 조건부 갱신의 **행 잠금**이 담당한다([7.2](#72-mysql-색인과-조건부-갱신)).

**경로별 직렬화 수단**

| 경로 | 경합 대상 | 직렬화 수단 |
|---|---|---|
| 구매 · 판매 취소 · 만료 배치 | `trade_listing` 등록 행 1개 | **MySQL 조건부 갱신**(`WHERE listing_id=? AND status=1`)의 행 잠금 |
| 판매 등록 | 그 계정의 **동시 등록 수** (잠글 행이 없다) | **Redis 판매자 단위 락** |
| 목록 조회 | 없음(읽기) | 없음 — 정합성 정본은 MySQL |

**등록 행을 닫는 경로에 락을 쓰지 않는 이유** — 경합 대상이 등록 행 하나이고, `status=1`일 때만 전이하는 UPDATE에 MySQL이 **그 행에 잠금을 걸어 요청을 줄 세운다**. 뒤에 온 요청은 조건이 어긋나 0행을 받고 `TradeAlreadyClosed(7005)`가 되므로, 이중 판매·아이템 복제가 성립할 여지가 없다. 앞단에 애플리케이션 락을 하나 더 두면 정합성은 그대로인데 다음 경계 상황만 늘어난다.

- **TTL 만료** — 트랜잭션이 TTL보다 오래 걸리면 두 요청이 동시에 락을 가진 상태가 되어, 어차피 DB 판정으로 돌아온다.
- **락 해제 실패·프로세스 사망** — TTL이 풀릴 때까지 그 등록의 구매가 막힌다(정합성이 아니라 가용성 손실).
- **Redis 장애** — 거래를 중단시키지 않으려면 락 없이 진행해야 하므로, 락이 있든 없든 최종 판정은 DB에 있어야 한다.

즉 락은 **혼잡 제어일 뿐 정합성 보증이 아니고**, 이 규모(단일 인스턴스·낮은 동시성)에서 줄여 줄 혼잡보다 늘어나는 실패 경계가 크다. 경합에서 진 요청도 **행 잠금 대기 후 UPDATE 1건**에서 즉시 판정되어 짧게 끝난다.

**판매 등록만 락을 쓰는 이유(확정)** — 등록 시점에는 아직 `listing_id`가 없고, 경합 대상이 **그 계정의 동시 등록 수**다. 한도 검사(`COUNT`)와 삽입 사이에 같은 계정의 다른 요청이 끼어들면 **한도(10건)를 넘길 수 있다**(9건에서 두 요청이 동시에 통과 → 11건). 잠글 대상 행이 없어 조건부 갱신으로는 막을 수 없으므로, 판매자 키로 그 계정의 등록을 직렬화한다.

| 항목 | 값(잠정) |
|---|---|
| 키 | `trade:lock:seller:{userId}` (판매 등록 전용) |
| 획득 방식 | `SET NX` + TTL(존재하지 않을 때만 성공) |
| TTL | 3초 — 락 보유 프로세스가 죽어도 자동 해제되어 계정이 영구히 잠기지 않는다 |
| 값 | 요청 식별자(UUID 등) — 해제 시 값이 자기 것일 때만 지운다(TTL 선만료 후 남의 락 해제 방지) |
| 재시도 | 50 ms 간격 2회 |
| 재시도 후에도 실패 | `TradeBusy(7008)` (HTTP 409) — 클라이언트가 잠시 후 재시도 |

- **보증이 아니라 완화다.** Redis 장애 시에는 락 없이 진행하므로(축소 운전, [7.5](#75-redis-장애와-캐시-불일치)) 경합이 다시 가능해진다. 초과 결과가 무해(한도가 스팸 방지 목적이고 1~2건 초과)하고 그 대가로 가용성을 지키는 선택이다. **DB 차원의 보증이 필요해지면** `(seller_user_id, slot_no)` 유니크 제약으로 슬롯을 배정하는 방식으로 바꿀 수 있다(테이블 추가 없이 컬럼 + 유니크 인덱스).
...
| 판매자 락 획득 실패(정상적인 경합) | 재시도 후 `TradeBusy(7008)`로 거부 — **같은 계정의 동시 등록을 여기서 차단** |
| 판매자 락을 쓸 수 없음(Redis 장애) | 락 없이 등록을 진행한다(축소 운전) — 동시 등록 한도가 1~2건 초과될 수 있으나 무해([7.4](#74-redis-락판매-등록-전용)) |
| 구매·취소·만료 | **Redis와 무관하다** — 직렬화·정합성이 MySQL 조건부 갱신 안에서 끝나므로 Redis 장애 중에도 정상 동작한다(캐시 제거만 실패, 아래 행) |
| 캐시 갱신 실패 | 거래는 이미 성립(커밋 완료)했으므로 되돌리지 않는다 — 되돌리려면 보상 트랜잭션이 필요하고, 그러면 Redis가 거래소의 단일 장애점이 된다. 스냅샷 선삭제 → 조회 시 감지 → **색인 재구축**으로 치유하고, 감지조차 안 되는 경우는 **색인 TTL 24시간**이 상한을 못 박는다([7.3](#73-redis-목록-캐시)) |
...
로그는 [로깅 규칙](../공통/로깅-규칙.md)을 따르고, 지표는 **목록 캐시 히트율 · MySQL 폴백 횟수 · `TradeAlreadyClosed` 발생률**(구매 경합 강도) · **`TradeBusy` 발생률**(등록 경합 강도) 넷을 본다.
...
| 의존성 수명 | 호스티드 서비스는 싱글턴이므로 scoped 리포지토리를 직접 주입받지 않고, **주기마다 `IServiceScopeFactory`로 스코프를 생성**해 `ITradeRepository`를 해석한다 | 목록 캐시 헬퍼(싱글턴, [7.3](#73-redis-목록-캐시))는 구매·취소와 공유 |
  ...
  트랜잭션(BEGIN)
    ...
    3) 반송 메일 발급(수신자 = L.seller_user_id):
         ...
         player_mail_reward 적재(seq=1, reward_type=2(아이템)|3(재료),
                                 reward_code=L.item_code, quantity=L.quantity)
  COMMIT
  캐시 제거(커밋 후, 7.3): trade:listing:{listingId} 삭제 → trade:index:{L.item_code}·trade:index:0에서 listingId 제거(스냅샷 먼저)
...
| Redis 장애 | 만료 처리 자체는 그대로 진행(락을 쓰지 않는다), 캐시 제거 실패는 무시(TTL·lazy 재적재가 흡수, [7.5](#75-redis-장애와-캐시-불일치)) | Warning(주기당 1회로 억제) |
...
| 후보 | 빼는 이유 |
|---|---|
| 구매·취소·만료에 Redis 락 두기 | 경합 대상이 등록 행 하나라 조건부 갱신의 행 잠금이 이미 직렬화한다. 락을 얹어도 정합성은 그대로인데 TTL 만료·해제 실패·Redis 장애라는 실패 경계만 늘어난다([7.4](#74-redis-락판매-등록-전용)) |
...
- `TradeBusy(7008)`는 [7.4](#74-redis-락판매-등록-전용)의 **판매자 락**(판매 등록 전용) 획득에 재시도까지 실패했을 때 쓴다. HTTP **409 Conflict**로 응답하며, 클라이언트는 잠시 후 재시도하면 성공할 수 있다. **판매 등록(5.2)에서만 나온다** — 구매·취소는 락을 쓰지 않으므로 이 코드를 반환하지 않는다.
- `TradeAlreadyClosed(7005)`와 **섞지 않는다**: 7005는 대상 등록이 이미 닫혀 재시도가 무의미한 상태(구매·취소 경로), 7008은 등록 요청이 계정 단위로 밀린 상태다.
- 동시 구매에서 진 요청은 7008이 아니라 `TradeAlreadyClosed(7005)`를 받는다(조건부 갱신 0행, [7.2](#72-mysql-색인과-조건부-갱신)).
...
|---|---|---|
| 판매자 락 TTL·재시도([7.4](#74-redis-락판매-등록-전용)) | TTL 3초 · 50 ms 간격 2회 재시도 | 등록 트랜잭션 실측 소요 시간(TTL은 그보다 넉넉해야 한다) |
...
>
> **확정 사항(성능·아키텍처)**: **Redis는 상시 사용**한다(데이터량과 무관 — **목록 캐시 + 판매 등록의 판매자 락**) · 다만 **단일 장애점으로 두지 않는다**(장애 시 목록은 MySQL 폴백, 등록은 락 없이 축소 운전) · **동시 구매·취소·만료 직렬화는 MySQL 조건부 갱신(행 잠금) 하나로 처리하고 Redis 락을 두지 않는다** · 정합성의 정본은 **MySQL 트랜잭션**이고 Redis에는 영속 데이터를 두지 않는다 · 캐시 갱신은 **커밋 이후**에만 수행 · 정렬 축은 **가격 오름차순 1축만** 유지한다 · 그 밖의 최적화는 [7.7](#77-하지-않는-것)에 따라 도입하지 않는다.
...
- [계정 / 로그인 기획서](account-login-기획서.md) — Redis 키 네이밍(`auth:token:{userId}`)·CloudStructures 사용 선례. 토큰 캐시는 없으면 인증 자체가 불가한 **필수 의존**이고, 거래소 Redis(목록 캐시·판매자 락)는 상시 사용하되 장애 시 축소 운전이 가능하다([7.5](#75-redis-장애와-캐시-불일치))
