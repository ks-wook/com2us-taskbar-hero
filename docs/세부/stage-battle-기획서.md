...
- **인벤토리가 가득 차 전리품을 적재할 수 없어도 클리어는 성공한다.** 이 경우 전리품만 폐기하고 골드·경험치는 그대로 지급하며, 응답의 `rewards.items`는 빈 배열, `inventoryDelta`도 비어 있다(6.3 확정).
...
- 전리품이 인벤토리 용량을 초과해도 **에러를 반환하지 않는다**(전리품 폐기 + 골드·경험치 지급, 6.3). 따라서 클리어에서 `InventoryFull(4002)`는 발생하지 않는다.
...
- **전리품 용량 초과 처리(확정)**: 인벤토리가 가득 차 전리품을 적재할 수 없으면 **전리품을 지급하지 않는다**(폐기, 메일 대체 없음). 이때 클리어를 에러로 거부하지 않고 **골드·경험치만 지급하고 진행도도 갱신**한다(전투를 다시 하게 만들지 않는다). → 본문 6.3.
