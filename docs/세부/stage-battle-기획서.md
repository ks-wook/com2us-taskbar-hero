...
- **Act·난이도 전진 순서(확정)**: **난이도-바깥** 순서를 채택한다 — Normal(난이도1)에서 Act1~3(각 4스테이지)을 모두 클리어하면 Hard(난이도2)가 열린다(`sequence = (difficulty-1)×12 + (act-1)×4 + stage`, 1~24). 현행 `StageCoords` 구현 그대로.
