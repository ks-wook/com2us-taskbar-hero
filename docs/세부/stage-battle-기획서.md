...
- **스테이지 구조**: 3 Act × 2 난이도 × 4 스테이지(=24)([마스터 데이터 기획서](master-data/master-data-기획서.md) `stage_master`). 각 Act·난이도는 스테이지 1~3 일반, 스테이지 4 보스다. 파티(3인)가 함께 하나의 스테이지를 진행한다. 진행도(현재 act/stage/difficulty)는 **계정/파티 단위**다.
