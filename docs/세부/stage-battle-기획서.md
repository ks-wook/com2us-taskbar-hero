...
- [5. API 명세](#5-api-명세)
  ...
  - [5.3 스테이지 실패 — `POST /api/game/stage/fail`](#53-스테이지-실패--post-apigamestagefail)
- [6. 처리 흐름](#6-처리-흐름)
- [7. 에러 코드](#7-에러-코드)
...
- **스테이지 실패(전멸)**: 파티가 전멸하면 클라이언트가 서버에 **실패를 보고**한다. 진행도·보상은 바뀌지 않고 **분석용 기록만 남는다** — 서버는 전투를 재현하지 않으므로, 이 보고가 없으면 "어느 스테이지에서 막히는가"를 `enter` 대비 `clear` 공백으로 *추정*할 수밖에 없다(추정은 앱 종료·이탈과 구분되지 않는다). 보고에는 좌표 외에 **얼마나 버텼는지·적을 얼마나 남겼는지·보스까지 갔는지**가 실려, 아슬아슬한 패배와 즉사를 갈라 본다([로그 이벤트 정의](../공통/로그-이벤트-정의.md) 5.3).
...
- 파티 전멸(실패) 보고를 접수해 기록한다. **어떤 상태도 바꾸지 않는다**(진행도·보상·재화 불변, 현재 진입 스테이지 유지 → 즉시 재도전 가능).
...
- **인벤토리가 가득 차 전리품을 적재할 수 없어도 클리어는 성공한다.** 이 경우 전리품만 폐기하고 골드·경험치는 그대로 지급하며, 응답의 `rewards.items`는 빈 배열, `inventoryDelta`도 비어 있다(6.4 확정).
- 오류: `StageNotEntered(6003)`(진입하지 않았거나 현재 진입 스테이지와 불일치).

### 5.3 스테이지 실패 — `POST /api/game/stage/fail`

현재 진입한 스테이지에서 **파티가 전멸했음**을 서버에 알린다. **기록 전용 엔드포인트**로, 진행도·보상·재화·인벤토리를 일절 바꾸지 않고 현재 진입 스테이지도 그대로 둔다(같은 스테이지 즉시 재도전).

**왜 두는가.** 전투가 클라이언트 권위라 서버는 전멸을 스스로 알 수 없고, 지금까지는 **`enter`는 있는데 `clear`가 없는 경우**를 실패로 추정해 왔다([로그 이벤트 정의](../공통/로그-이벤트-정의.md) 5.3). 이 추정은 **진짜 전멸**과 **앱 종료·네트워크 끊김·진행 중 이탈**을 구분하지 못해, 실질 난이도가 실제보다 나빠 보인다. 실패를 직접 받으면 분자(실패)를 세어 난이도를 관측할 수 있고, 남은 두 필드로 **얼마나 아슬아슬했는지**까지 본다 — 리밸런싱이 필요한 스테이지를 고르는 근거가 된다.
...
```json
{
  "userId": 1,
  "token": "...",
  "data": { "act": 2, "difficulty": 1, "stage": 15, "elapsedMs": 41200, "remainingMonsterCount": 2, "reachedBoss": true }
}
```

- 요청의 스테이지는 클리어와 마찬가지로 **현재 진입 스테이지와 일치**해야 한다. 진입하지 않은 스테이지의 실패 보고를 받으면 난이도 집계의 분모(`enter`)와 짝이 맞지 않는다.
- `elapsedMs`: 진입부터 전멸까지 걸린 시간(ms). **즉사(벽)와 접전을 가른다.**
- `remainingMonsterCount`: 전멸 시점에 남아 있던 적 수. **0에 가까울수록 아슬아슬한 패배**이므로, 소폭 하향으로 넘길 수 있는 스테이지를 찾는 축이다.
- `reachedBoss`: 보스전까지 갔는지. **잡몹이 벽인지 보스가 벽인지**를 가른다(보스가 없는 스테이지는 항상 `false`).
- 세 값은 모두 **클라이언트가 보고하는 관측치**이며 서버가 검증할 수단이 없다. 어떤 이득도 걸려 있지 않아(보상·진행도 불변) 조작 동기가 없으므로 분석용으로 그대로 받되, **음수는 클라이언트 결함으로 보고 거부**한다(`InvalidRequest(1006)`).
...
```json
{
  "success": true,
  "errorCode": 0,
  "message": "Stage failure recorded",
  "data": { "act": 2, "difficulty": 1, "stage": 15, "stageId": 1020015, "failedAt": 1752343241 }
}
```

- 응답은 **접수 확인**이 전부다. 바뀐 상태가 없으므로 `rewards`·`progress`·`inventoryDelta`를 담지 않으며, 클라이언트는 이 응답으로 캐시를 갱신하지 않는다.
- 오류: `StageNotFound(6001)`, `StageNotEntered(6003)`(진입하지 않았거나 현재 진입 스테이지와 불일치), `InvalidRequest(1006)`(보고 수치가 음수).
- **실패 보고는 재시도하지 않아도 된다.** 유실돼도 게임 상태에 영향이 없고 집계 표본이 한 건 줄 뿐이므로, 클라이언트는 응답을 기다리지 않고 다음 전투를 시작해도 된다.
...
트랜잭션(BEGIN, user_id 잠금)
  ...
  3) 지급: player_item(재화, item_code=골드).quantity += gold
           ...
           items를 player_item에 적재(스택/용량 규칙; 칸이 없으면 적재 생략 = 전리품 폐기, 클리어는 계속)
...
요청 수신 → 토큰 검증(미들웨어)
  1) st = stage_master[act, difficulty, stage]      # 없으면 StageNotFound(6001)
  2) elapsedMs < 0 또는 remainingMonsterCount < 0 이면 InvalidRequest(1006)
  3) 요청 스테이지가 game_player 현재 진입 스테이지와 일치?  아니면 StageNotEntered(6003)
  4) 실패 이벤트 기록(stage.fail) — 좌표·elapsedMs·remainingMonsterCount·reachedBoss
     # 쓰기 없음: 진행도·재화·인벤토리·현재 진입 스테이지 모두 그대로 둔다(트랜잭션 불필요).
→ { act, difficulty, stage, stageId, failedAt }
```

### 6.4 예외 / 엣지 케이스

- **스킵 진입/클리어**: 도달하지 못한 스테이지는 `StageLocked(6002)`(진입)·`StageNotEntered(6003)`(클리어 불일치)로 거부.
- **전리품 용량 초과**: `stack_max`까지 채우고 초과분은 새 행 분할, 적재할 공간이 없으면 **전리품만 폐기하고 클리어는 성공 처리**한다 — 골드·경험치·진행도는 정상 반영하며 에러로 거부하지 않는다(8장 확정). 폐기된 전리품은 메일로 대체 지급하지 않고, 응답 `rewards.items`·`inventoryDelta`에서 제외해 클라이언트가 획득하지 않은 것으로 표시한다. 적재 실패는 빈 칸 판정 단계에서 결정되므로 부분 적재는 발생하지 않는다.
...
- **신규 세이브의 첫 스테이지는 "미진입"과 구분되지 않는다.** 현재 진입 스테이지를 `game_player.act`/`difficulty`/`stage`로 나타내고 신규 세이브의 기본값이 `1-1-1`이라, **한 번도 진입하지 않은 계정의 `1-1-1` 실패 보고도 접수된다**(클리어도 같은 성질이다). 별도 "미진입" 상태값을 두지 않기로 한 데서 오는 한계이며, 첫 스테이지 한 칸의 집계 오차라 감수한다.
- **실패 보고 중복·유실**: 실패 보고는 상태를 바꾸지 않으므로 **멱등성을 따로 보장하지 않는다** — 같은 전투를 두 번 보고하면 로그 행이 둘 생길 뿐이고, 유실되면 표본 한 건이 준다. 재시도·중복 제거 장치를 두지 않는다(비용 대비 이득 없음).
- **실패 뒤 클리어 보고**: 실패는 진입 상태를 지우지 않으므로 같은 스테이지의 `clear`가 곧이어 올 수 있다(재도전 성공). 정상 흐름이며 서버는 두 사실을 그대로 남긴다 — 재도전 횟수는 두 이벤트를 시각순으로 짝지어 센다.
...
- **실패 보고(5.3)는 새 에러 코드를 만들지 않는다** — 진입·클리어와 같은 `StageNotFound(6001)`·`StageNotEntered(6003)`을 쓰고, 보고 수치가 음수인 클라이언트 결함만 공통 `InvalidRequest(1006)`으로 거부한다. "전투에서 졌다"는 사실 자체는 **오류가 아니라 정상 응답**(`errorCode = 0`)이다.
- 전리품이 인벤토리 용량을 초과해도 **에러를 반환하지 않는다**(전리품 폐기 + 골드·경험치 지급, 6.4). 따라서 클리어에서 `InventoryFull(4002)`는 발생하지 않는다.
...
- **전리품 용량 초과 처리(확정)**: 인벤토리가 가득 차 전리품을 적재할 수 없으면 **전리품을 지급하지 않는다**(폐기, 메일 대체 없음). 이때 클리어를 에러로 거부하지 않고 **골드·경험치만 지급하고 진행도도 갱신**한다(전투를 다시 하게 만들지 않는다). → 본문 6.4.
- **최종 스테이지 이후 처리(확정)**: 마지막 스테이지(최종 Act·난이도의 보스 스테이지)를 클리어하면 다음 진행처가 없으므로 진행도를 전진시키지 않고 **그 최종 스테이지에 머물며 계속 재파밍**한다. → 본문 6.4.
- **Act·난이도 전진 순서(확정)**: **난이도-바깥** 순서를 채택한다 — Normal(난이도1)에서 Act1~5(각 10스테이지)을 모두 클리어하면 Hard(난이도2)가 열린다(`sequence = (difficulty-1)×50 + (act-1)×10 + stage`, 1~100). 현행 `StageCoords` 구현 그대로.
...
- **실패(전멸) 보고(확정)**: 승패 판정도 클라이언트가 하므로, **패배 역시 클라이언트가 보고**한다(`POST /api/game/stage/fail`). 기록 전용이라 **어떤 상태도 바꾸지 않으며**, 보상이 걸려 있지 않아 검증 없이 받는다. 이 보고가 있어야 실질 난이도를 `enter` 공백으로 추정하지 않고 **직접 측정**할 수 있다. → 본문 5.3.
...
- [로그 이벤트 정의](../공통/로그-이벤트-정의.md) — 5.3 스테이지/전투 이벤트(`stage.enter`·`stage.clear`·`stage.fail`)와 실질 난이도 측정
