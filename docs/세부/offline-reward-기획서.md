...
>
> 본 문서는 플레이어가 미접속(오프라인)한 동안의 자동 진행분을 재접속 시 **서버 권위로 정산**하는 규칙을 다룬다. 기준 시각·저장 구조는 [세이브 데이터 기획서](save-data-기획서.md), 산출에 쓰는 정적 수치는 [마스터 데이터 기획서](master-data-기획서.md)를 참고한다.
...
- **감사 로그**: 정산 이력(재화·경험치 지급 원장) 관리는 본 문서에서 별도로 두지 않고, **추후 재화/상점/결제 시스템 기획서**에서 재화 지급 원장의 일부로 다룬다.
...
> 확정된 항목: 최대 누적 시간 12시간, 최소 정산 시간 10분, 오프라인 효율 50%, 아이템 미지급(2·6장), **`OfflineRewardResult` DTO 필드(8장)**. 정산 감사(재화 지급 원장)는 재화/상점/결제 기획서로 이관.
...
- [마스터 데이터 기획서](master-data-기획서.md) — 스테이지·몬스터·드롭 테이블(산출 근거)
