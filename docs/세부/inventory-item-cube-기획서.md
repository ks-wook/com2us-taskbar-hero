...
**Redis를 쓰는 곳과의 경계** — 거래소 목록도 **전역 공유 읽기**지만 결국 캐시를 두지 않기로 확정했다(전용 색인이 검색·정렬을 커버해 캐시가 읽기를 더 싸게 만들지 못했다, [거래소 기획서](trade-기획서.md) 7.3). 인증 토큰은 Redis를 계속 쓰고, **보스러시 랭킹 조회용 Redis 키 3종**([보스러시 기획서](boss-rush-기획서.md) 4.3)이 여기에 더해진다 — 리더보드 `rank:bossrush:{seasonId}`(Sorted Set)는 순위가 "정렬된 전체 집합에서의 위치"라 개인 행 조회로 답할 수 없어 파생 인덱스가 불가피하고, 그 응답을 완성하는 `player:nickname`·`bossrush:season:current`(Hash 2종)는 **전역 공유 읽기에 붙는 캐시**여서 위 표의 조건을 충족한다(랭킹은 뜨거운 경로이고, 두 값 모두 사실상 불변이라 동기화 위험이 없다). 즉 "Redis를 쓰지 않는다"가 아니라 **"개인 데이터의 가벼운 조회에는 캐시를 두지 않는다"** 는 판단이다.
