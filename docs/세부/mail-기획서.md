...
2. **렌더링과 적재의 분리**: 발급 헬퍼(`GameServer/Util/MailUtil.cs`)가 인메모리 `mail_master`로 제목·본문을 렌더링해 메일 초안을 만들고, `player_mail` + `player_mail_reward` INSERT는 **발급자의 트랜잭션 안에서** 리포지토리가 수행한다. 거래소 구매(선점 + 골드 차감 + 대금 메일), 출석 획득(출석 기록 + 보상 메일), 보스러시 시즌 정산(순위 확정 + 보상 메일)이 각각 하나의 트랜잭션으로 원자성을 유지하는 구조가 그대로 성립한다.
