...
--    목표 클리어 시간: 일반 스테이지 15초 내외, 보스 스테이지 15~20초(Act1~3은 대충 갖춰도 막히지 않는 구간).
--    Act4~5만 20~40초로 늘려 성장·장비 투자를 요구한다. 보스(xx99) hp는 같은 Act 최강 일반 몬스터의 7~17배.
-- attack 산정 기준(값 문서 §9): 아군 피해 = max(1, attack − 아군 방어력)(정액 감산)이므로, attack은
--    "최약체(마법사, 해당 Act 표준 레벨 + 한 티어 낮은 장비)가 평타 몇 대를 버티는지"로 역산한다.
--    목표 피격 횟수: Act1~2 넉넉(20대 이상) · Act3 약 11대 · Act4 약 7대 · Act5 약 5대(후반만 의도적으로 어렵게).
--    보스(xx99)는 hp만 압도적으로 높고 attack은 같은 Act 최강 일반 몬스터의 1.3~1.45배로 억제한다
--    (아군 즉사 방지 — 보스는 "한 방이 아픈 적"이 아니라 "오래 버티는 적").
