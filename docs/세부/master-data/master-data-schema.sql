...
--    같은 대역의 12로 두고, 탱커의 내구는 hp 1300(2위 슬레이어 860의 1.51배)과 방패 독점으로만 표현한다.
--    내구 대역은 사거리로 가른다 — 근접(기사 1300 · 슬레이어 860)이 동시 5마리를 받아내고 원거리(레인저
--    540 · 마법사 470)는 덜 맞는다. 슬레이어 hp 860 · def 11은 '기사 다음으로 튼튼한 근접 딜러' 선이며
--    (유효 체력 955 = 기사 1,456의 65.6%), 방패를 못 껴 장비로 내구를 보태지 못하는 근접이 1지역에서
--    간헐적으로 죽던 문제를 여기서 해소한다. def는 11로 둔다 — 12로 올리면 기사와 같아진다(값 문서 §2·§9.2).
-- =====================================================================
...
INSERT INTO class_master (class_code, name, description, unlock_type, hp, atk, def, move_speed, crit_chance, crit_damage, cooldown) VALUES
    ...
    (4, '슬레이어', '거대한 도끼를 휘둘러 적을 베어 넘기는 공격 특화 근접 전사다.',        0, 860, 15, 11, 3.4, 0.12, 1.6, 1.1);
