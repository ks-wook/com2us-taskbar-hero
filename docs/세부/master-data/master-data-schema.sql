...
INSERT INTO class_master (class_code, name, description, unlock_type, hp, atk, def, move_speed, crit_chance, crit_damage, cooldown) VALUES
    ...
    (4, '슬레이어', '거대한 도끼를 휘둘러 적을 베어 넘기는 공격 특화 근접 전사다.',        0, 130, 15, 9, 3.4, 0.12, 1.6, 1.1);
...
-- hp 산정 기준(값 문서 §9): 몬스터는 방어력이 없어 아군 피해 = 공격력이므로, hp는 "파티가 스테이지 하나를
--    미는 데 걸리는 시간"으로 역산한다(방치형이라 스테이지가 분 단위로 늘어지면 안 된다).
--    기준 파티: 3캐릭터·해당 Act 표준 레벨(Act1 8/Act2 27/Act3 50/Act4 80/Act5 100)·"대충 낀" 장비
--    (무기만 두 티어 낮은 등급, 스킬·룬 투자 없음, 치명타 무시) → 파티 DPS 근사 100/225/340/530/690.
--    목표 클리어 시간: 일반 스테이지 10초 내외, 보스 스테이지 10~20초.
--    Act1은 별도 기준 — "신규 계정 1레벨·장비 없음" 3인 파티(DPS 약 37)가 1-1(약몹 8마리)을 깰 수 있어야 한다.
--    보스(xx99) hp는 같은 Act 최강 일반 몬스터의 7~17배.
-- attack 산정 기준(값 문서 §9): 아군 1대 피해 = max(1, round(attack × 2 × 100/(100+아군 def)))다
--    (적 피해 배수 2 + 방어력 비율 경감, 반감점 100 — 정액 감산이 아니므로 피해가 1로 바닥치지 않는다).
--    몬스터는 1.5초 주기로 때리고 동시 최대 5마리가 최전방 아군 1명에게 붙으므로, 파티 생존 시간이
--    스테이지 클리어 소요 시간의 2배 이상이어야 회복 수단 없이 진행된다.
--    단 Act5는 최종 구간이라 의도적으로 1.3~1.4배로 좁혀 둔다(보스전 긴장감).
--    보스(xx99)의 attack은 같은 Act 최강 일반 몬스터의 1.3~1.45배로 억제한다(아군 즉사 방지).
--    보스 스테이지 최악 조건은 "보스 + 일반 4마리 동시"(동시 상한 5)로 계산한다.
...
--      일반 몬스터 2종인 Act1·2 — 스테이지가 오를수록 약몹이 강몹으로 교체된다(1: 약 8 → 9: 약 3+강 9, 보스 10: 강 5).
--      일반 몬스터 1종인 Act3~5 — 스테이지 1~9는 8→16마리로 증가, 보스 스테이지(10)는 6마리.
...
--    값 공식(값 문서 §10): reward_gold = base(act, difficulty) × stage 배율, reward_exp = reward_gold ÷ 2.
--      base(난이도1/난이도2): Act1 100/250 · Act2 800/2,000 · Act3 6,000/15,000 · Act4 40,000/100,000 · Act5 250,000/600,000
...
INSERT INTO stage_reward (stage_id, reward_gold, reward_exp) VALUES
    ...
    (4010007,    88000,   44000),
    (4010008,    96000,   48000),
    (4010009,   104000,   52000),
    (4010010,   200000,  100000),
    (4020001,   100000,   50000),
    (4020002,   120000,   60000),
    (4020003,   140000,   70000),
    (4020004,   160000,   80000),
    (4020005,   180000,   90000),
    (4020006,   200000,  100000),
    (4020007,   220000,  110000),
    (4020008,   240000,  120000),
    (4020009,   260000,  130000),
    (4020010,   500000,  250000),
    (5010001,   250000,  125000),
    (5010002,   300000,  150000),
    (5010003,   350000,  175000),
    (5010004,   400000,  200000),
    (5010005,   450000,  225000),
    (5010006,   500000,  250000),
    (5010007,   550000,  275000),
    (5010008,   600000,  300000),
    (5010009,   650000,  325000),
    (5010010,  1250000,  625000),
    (5020001,   600000,  300000),
    (5020002,   720000,  360000),
    (5020003,   840000,  420000),
    (5020004,   960000,  480000),
    (5020005,  1080000,  540000),
    (5020006,  1200000,  600000),
    (5020007,  1320000,  660000),
    (5020008,  1440000,  720000),
    (5020009,  1560000,  780000),
    (5020010,  3000000, 1500000);
