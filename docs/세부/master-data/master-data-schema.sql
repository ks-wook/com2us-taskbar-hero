...
--    hp·attack 은 레벨 1 기준값이다 — 실제 등장 레벨은 stage_spawn.monster_level 이 정하고,
--    전투 스탯은 클라이언트가 레벨 배율을 곱해 산출한다(값 문서 §9.4).
-- =====================================================================
...
--        1지역은 끝까지 클리어(체력 85%→23%로 계속 닳는다), 2지역은 3스테이지부터 전사·6스테이지부터
--        전멸, 3지역 이후는 1스테이지부터 전멸한다. 이 지점이 장비작·스킬작을 요구하는 벽이다.
-- hp 산정 기준(값 문서 §9.2): 몬스터는 방어력이 없어 아군 피해 = 공격력이므로, hp는 위 목표 시간에서
--    역산한다. 소요 시간을 정하는 것은 "스테이지 총 hp = Σ(마리 수 × hp)"이므로 stage_spawn과 함께 맞춘다.
--    지역 안의 상승은 Act1·2가 약몹→강몹 교체(강몹 hp = 약몹 × 2.5), Act3~5가 마리 수 증가(8→17).
--    보스(xx99) hp는 같은 Act 최강 일반 몬스터의 9~13배 — 보스 스테이지는 "보스 1 + 강몹 8" 구성이다.
...
--    기준 파티 9스테이지 최저 체력 46/49/35/34/30%, 보스 스테이지 23/22/29/19/15%(Act1~5).
--    보스(xx99)의 attack은 스테이지 전체 압력에서 역산한다 — 보스 스테이지는 강몹 8마리가 60초 내내
--    붙으므로 보스까지 비례로 올리면 즉사한다. 같은 Act 최강 일반 몬스터의 0.6~1.6배로 지역마다 다르다.
...
--      monster_master 의 레벨 1 기준값에 레벨 배율을 곱해 클라이언트가 산출한다(값 문서 §9.4).
--      hp = round(base_hp × 1.25^(level-1)) / attack = round(base_attack × 1.18^(level-1)).
--    10 스테이지 구성의 스폰 곡선(값 문서 §11-B):
--      일반 몬스터 2종인 Act1·2 — 스테이지가 오를수록 약몹이 강몹으로 교체된다(1: 약 16 → 9: 강 19, 보스 10: 강 8).
--        강몹 hp가 약몹의 2.5배라 교체만으로 스테이지 총 hp가 1스테이지 대비 9스테이지에서 약 3배가 된다.
--      일반 몬스터 1종인 Act3~5 — 스테이지 1~9는 8→17마리로 증가, 보스 스테이지(10)는 8마리 + 보스 1.
--      마리 수는 몬스터 hp·레벨과 함께 목표 클리어 시간(30초→50초, 보스 60초)을 만드는 축이다(값 문서 §9.1).
