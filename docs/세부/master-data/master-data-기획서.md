...
- **슬레이어(4)**: 도끼를 쓰는 **근접 딜러**. 같은 근접인 기사보다 체력·방어가 낮고(hp 860 vs 1300, def 11 vs 12) 공격력·치명확률·평타 속도가 높은 **공격 특화** 직업이다. 내구는 전 직업 2위로, **근접이라 원거리(레인저 540·마법사 470)보다는 튼튼하고 탱커(기사)보다는 낮은** 중간 대역에 둔다. 전용 무기는 도끼(`31·4·등급·순번`), 보조무기는 방패가 아닌 **사슬 갈고리**(`32·4·등급·순번`)라 방어 옵션을 얻지 못한다. 값 정본은 [마스터 데이터 값](master-data-값.md) §2·§4·§6.
...
| `stat_multiplier` | decimal(5,3) | 이 단계에서 장비 옵션 스탯(`base_stats`) **전체**에 곱할 배율(`1.200`=120%, +10은 `3.000`=300%) |
...
- **확정 배너 2종**: `60001` 상시 뽑기(기간 없음, 5등급 슬롯 = 전설 장비 20종), `60002` 성검 엑스칼리버 픽업(한정 89일, 5등급 슬롯 = `31151` 1종).
...
- **`character_create_cost`(신규·부가 테이블)**: 캐릭터 추가 생성 골드 비용 테이블(`character_id`·`gold_cost`). `character_id`는 파티 슬롯이 아니라 **생성 순번**(2~)이며, 최초 캐릭터(순번 1 = 계정 초기화)는 무료라 행이 없다. 값·정책 정본은 [마스터 데이터 값](master-data-값.md) 부록과 [세이브 데이터 기획서](../save-data-기획서.md) 5.3을 따른다(**현재 순번별 누진 5,000 / 50,000 / 400,000골드** — 지역별 기준 파티 인원 1→2→3명에 맞춘 값, 학습용 임시값). 골드는 생성 시에만 소모하고 **파티 편성 저장은 무료**다. 클라이언트가 같은 번들로 생성 전 비용을 안내한다.
