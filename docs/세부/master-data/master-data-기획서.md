...
- **슬레이어(4)**: 도끼를 쓰는 **근접 딜러**. 같은 근접인 기사보다 체력·방어가 낮고(hp 720 vs 1300, def 11 vs 12) 공격력·치명확률·평타 속도가 높은 **공격 특화** 직업이다. 내구는 전 직업 2위로, **근접이라 원거리(레인저 540·마법사 470)보다는 튼튼하고 탱커(기사)보다는 낮은** 중간 대역에 둔다. 전용 무기는 도끼(`31·4·등급·순번`), 보조무기는 방패가 아닌 **사슬 갈고리**(`32·4·등급·순번`)라 방어 옵션을 얻지 못한다. 값 정본은 [마스터 데이터 값](master-data-값.md) §2·§4·§6.
