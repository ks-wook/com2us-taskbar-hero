...
- **날짜 경계·월 리셋**: 출석 일자는 **서버 기준 KST(UTC+9) 자정**으로 나뉜다. 출석 현황은 **월 단위**이며, 달이 바뀌면 새 달의 달력으로 초기화된다(과거 달 기록은 남되 조회 기본은 이번달).
...
- 획득한 보상은 **메일로 발급**한다([메일 기획서](mail-기획서.md) `player_mail`·`player_mail_reward`, `category=3`(출석)).
...
> 인증 오류(401) 등은 기존 미들웨어를 따른다. 다른 달 조회·과거 미수령 소급 수령은 8장 미결.
...
트랜잭션(BEGIN, user_id 잠금)
  ...
  4) 보상 메일 발급:
       INSERT player_mail(user_id, category=3, title/body, created_at=now, expires_at, claimed=0)
...
- 발급 메일의 `expires_at`(출석 보상 메일 만료 기간)은 [메일 기획서](mail-기획서.md) 8장 만료 정책을 따른다(기본값 미결).
...
- **월 리셋**: 달이 바뀌면 이번달 현황은 빈 달력으로 시작한다(과거 달 `player_attendance` 행은 유지). 미수령 과거일 소급 수령은 8장 미결.
...
- **연속/누적 출석 보너스**: 개요 4.10의 "연속 출석/누적 출석 보상". 현재 설계는 **일자별(day-of-month) 단순 출석**만 다룬다. 연속 출석 스트릭 보너스 도입 시 스트릭 카운트 저장(신규 컬럼/테이블)과 별도 보상 테이블·에러 코드를 확정한다.
- **과거일 소급 수령**: 미접속으로 놓친 과거 일자를 이번달 내에 소급 수령 가능하게 할지(현재는 오늘만 수령). 허용 시 `claim`에 대상 일자 파라미터 추가.
- **다른 달 조회**: 지난달 출석 현황 조회 필요 여부(현재 조회는 이번달만).
- **출석 보상 메일 만료**: 발급 메일의 기본 만료 기간(메일 기획서 8장 만료 정책과 연계).
- **타임존 확정**: 날짜 경계 KST(UTC+9)를 잠정 확정. 글로벌 서비스 시 서버 타임존/사용자 로컬 타임존 정책 재검토.
