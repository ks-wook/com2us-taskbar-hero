...
- **배치 중복 실행 방지**: 시즌 정산 배치는 **BatchServer**(주기 배치 전담 워커) 1대에서만 돌므로 분산 락을 쓰지 않는다. 발화 시각은 진행 중 시즌의 `end_at`(절대 시각)이다. **랭킹 캐시 적재는 배치가 아니라 게임 서버**가 하며(기동 시 관리 API 호출 + 조회가 미적재를 감지했을 때의 자동 재적재), 게임 API는 scale-out으로 N대가 뜨므로 **여기에는 락을 쓴다**(6.3).
...
**적재 완료 마커** — `rank:bossrush:{seasonId}:ready` (String)

- **이 키가 있어야 조회가 리더보드를 쓴다.** 워밍업이 시즌 기록을 **전량** 적재했을 때만 세워지고, 재적재를 시작할 때와 클리어 보고의 ZADD가 실패했을 때 내려간다(6.3).
- **리더보드 키의 존재로는 판단할 수 없기 때문에 따로 둔다.** 없는 키의 `ZCARD`는 0을, `ZRANGE`는 빈 목록을 돌려주므로 캐시 부재와 "등재 0명"이 구별되지 않고, 클리어 보고의 ZADD가 키를 새로 만들 수 있어 **한 명만 든 리더보드**가 정상처럼 보인다.
- **TTL**: 시즌 종료 시 리더보드와 **같은 7일 TTL**을 건다 — 리더보드만 사라지고 마커가 남으면 그 시즌 조회가 빈 랭킹을 정상값으로 믿게 된다.

**재적재 락** — `rank:bossrush:{seasonId}:rebuilding` (String, `SET NX` + TTL 2분)

- 자동 재적재가 **인스턴스마다 중복으로 돌지 않게** 한다(게임 API는 N대). 잡은 하나만 적재하고, 못 잡았으면 조용히 넘어간다(다음 조회가 다시 건다).
- **TTL로 스스로 풀린다** — 적재 도중 프로세스가 죽어 락을 반납하지 못해도 2분 뒤 다시 시도할 수 있다. 적재가 그보다 오래 걸려 두 인스턴스가 겹쳐도 ZADD는 member 단위 덮어쓰기라 결과가 같다.
...
랭킹 캐시(리더보드 ZSET + 시즌 메타)를 정본에서 적재한다. **게임 클라이언트가 부르는 API가 아니다** — 호출 주체는 부트스트랩 스크립트 `python server_up_with_docker.py`다(6.3).

기동 절차에서 쓰는 경로이며, **평상시 복구는 이 API를 기다리지 않는다** — 조회가 미적재 리더보드를 만나면 스스로 재적재를 건다(6.3). 이 엔드포인트는 기동 시의 명시적 적재와, 캐시를 강제로 다시 만들어야 할 때의 운영 수단이다.

**인증**: 게임 토큰이 아니라 `X-Admin-Key` 헤더(설정 `Admin:ApiKey`). 키가 설정돼 있지 않으면 엔드포인트가 **404로 닫힌다**.

**요청**: body 없음. 쿼리 `?force=true`(선택)면 **적재 완료 마커가 있어도** 다시 적재한다.
...
- `status`: `no-season`(진행 중 시즌 없음) · `already-warm`(**적재 완료 마커가 있어** 건너뜀) · `restored`(MySQL에서 재구축) · `cache-unavailable`(Redis 접근 실패 또는 부분 적재).
- `restored`: 이번 호출에서 리더보드에 넣은 기록 수. `members`: 적재 후 등재 인원(`ZCARD`). 재적재는 **비우고 채우므로** 성공하면 두 값이 같다 — 다르면 적재 중 실패가 있었다는 뜻이다.
- HTTP 상태: `200` 성공 · `401` 키 불일치 · `404` 관리 API 미설정 · `503` `cache-unavailable`(스크립트가 실패로 본다).
- **멱등**하다 — 몇 번을 호출해도 결과가 같다(정본에서 통째로 다시 만들고, 점수는 기록에서 결정론적으로 계산된다).
...
COMMIT
  5) 커밋 이후(랭킹 반영·순위 산출):                                    # ★ 커밋 후
     if isNewRecord:
        ZADD rank:bossrush:{seasonId} score(bestClearMs, recordedAt − season.start_at) userId
        실패하면 DEL rank:bossrush:{seasonId}:ready                     # 나만 빠진 리더보드를 믿지 않게 한다
     rank = 마커가 있으면 ZRANK + 1, 없으면 boss_rush_record에서 계산   # 갱신 후 상태에서 순위 계산
...
- **기록을 갱신하지 못했어도 `rank`는 내려준다.** `isNewRecord`가 `false`면 ZADD만 건너뛰고 순위 산출은 수행하므로 `rank`는 항상 **기존 최고 기록 기준 현재 순위**다.
- **ZADD 실패는 그냥 넘기지 않는다.** 기록은 정본에 들어갔는데 리더보드에만 빠진 상태라 리더보드가 멀쩡해 보이고, 등재는 `isNewRecord`일 때만 하므로 **자기 최고 기록을 넘어서기 전까지 그 사람은 랭킹에서 사라진다**. 적재 완료 마커를 내려 다음 조회가 폴백·재적재하게 하고(6.3), 이 응답의 `rank`도 정본에서 계산한다.
...
- **리더보드를 믿어도 되는지는 적재 완료 마커가 정한다.** `rank:bossrush:{seasonId}:ready`가 있을 때만 조회가 캐시를 쓰고, 없으면 정본으로 폴백한다. **리더보드 키의 존재로 판단하지 않는다** — 없는 키의 `ZCARD`·`ZRANGE`는 예외가 아니라 "0명·빈 목록"을 돌려주므로, 그대로 쓰면 캐시가 비었을 때 **빈 랭킹이 정상 응답으로 나간다**(오답이지 축소 운전이 아니다). 게다가 클리어 보고의 ZADD가 키를 새로 만들 수 있어, Redis를 재기동한 뒤 한 명이 기록을 갱신하면 **그 한 명만 든 리더보드**가 생긴다 — 비어 있는 것보다 나쁜 상태(틀린 순위표가 정상으로 보인다)이며, 마커가 없으므로 이 역시 폴백 대상이 된다.
- **캐시 적재(워밍업)에는 경로가 둘이다.** 둘 다 같은 서비스를 태우며, `boss_rush_record`를 페이지 단위(500)로 읽어 ZADD로 재구축하고(점수 인코딩에 그 시즌의 `start_at`이 필요하므로 시즌 행을 함께 들고 있는다, 4.3) `bossrush:season:current`도 같은 호출에서 적재한다. **닉네임 캐시는 적재 대상이 아니다** — lazy로 채워진다(4.3).
  - **① 기동 절차** — 관리 API `POST /api/admin/boss-rush/rank/warmup`(`X-Admin-Key` 헤더). 호출 주체는 부트스트랩 스크립트(`python server_up_with_docker.py`)이며, 컨테이너·서버를 띄우고 헬스 체크를 통과한 뒤 한 번 호출한다.
  - **② 자동 재적재** — 조회가 마커 없는 리더보드를 만나면 그 요청은 정본 폴백으로 정답을 내려주고, **백그라운드로 재적재를 건다.** 첫 조회가 캐시를 복구하므로 다음 요청부터는 캐시 경로로 돌아온다.
  - **왜 자동 경로가 필요한가**: 리더보드에 기록이 들어가는 경로는 워밍업과 클리어 보고의 ZADD뿐이고, **조회는 캐시를 채우지 않는다.** 적재가 기동 절차뿐이면 Redis를 껐다 켠 뒤 사람이 관리 API를 다시 부를 때까지 랭킹이 비어 있고, 그동안 아무도 이상을 알아채지 못한다(에러도 로그도 없이 200으로 나간다). 클라이언트가 새로고침해도 같은 응답을 다시 받을 뿐이다.
  - **중복 재구축은 락으로 막는다.** 자동 경로가 생겨 "호출자가 하나"라는 전제가 깨졌고, **게임 API는 scale-out으로 N대가 뜬다** — 마커 부재를 동시에 감지한 인스턴스들이 저마다 적재를 시작하지 않도록 `rank:bossrush:{seasonId}:rebuilding`을 `SET NX`(TTL 2분)로 잡은 하나만 적재한다. 락을 잡지 못했거나 Redis를 쓸 수 없으면 조용히 넘어가고, 다음 조회가 다시 건다. (인스턴스 1대 고정이라 락을 쓰지 않는 BatchServer와 다른 점이다.)
  - **재적재는 진행 중 시즌에만 건다.** 종료된 시즌은 TTL로 캐시가 사라지는 것이 정상이라 되살릴 이유가 없고, 그대로 두면 과거 시즌을 열어 볼 때마다 재적재가 걸린다.
  - **비우고 채운다.** ZADD는 멤버를 갱신할 뿐 **정본에서 사라진 멤버를 지우지 않으므로**, 덮어쓰기만 하면 유령 멤버가 남아 순위를 밀어낸다. 그래서 재적재는 `마커 내리기 → 리더보드 비우기 → 전량 ZADD → 마커 세우기` 순서로 돈다. 중간에 끊겨도 마커가 없으니 조회는 정본으로 폴백하고 다음 재적재가 다시 시도한다.
  - **몇 번을 호출해도 안전하다**(멱등). 정본이 MySQL이고 점수는 기록에서 결정론적으로 계산된다. `?force=true`면 마커가 있어도 다시 적재한다.
  ...
  - **응답 `status`**: `no-season`(진행 중 시즌 없음) · `already-warm`(적재 완료 마커가 있어 건너뜀) · `restored`(재구축) · `cache-unavailable`(Redis 접근 실패 또는 부분 적재 — HTTP 503, 스크립트가 실패로 본다).
- **폴백(축소 운전)**: 리더보드를 믿을 수 없으면(마커 부재·Redis 접근 실패) 랭킹 조회를 MySQL로 처리한다 — `SELECT ... ORDER BY best_clear_ms, recorded_at LIMIT ? OFFSET ?`가 `(season_id, best_clear_ms, recorded_at)` 인덱스를 그대로 타므로 정렬·필터가 인덱스 안에서 끝난다. 내 순위는 `COUNT(*) WHERE (best_clear_ms, recorded_at) < (내 값)` + 1로 계산한다. 응답 `source=2`로 알린다.
...
- **캐시가 정본을 앞서지 않는다**: ZADD는 언제나 커밋 이후에만 한다(6.2 5단계).
  - **ZADD 실패를 버리지 않는다.** 기록은 정본에 들어갔는데 리더보드에는 그 사람만 빠진 상태이고, 리더보드 자체는 멀쩡해 보이므로 아무도 폴백하지 않는다 — 그대로 두면 **다음 기록 갱신 전까지 그 사람은 랭킹에서 사라진다**(등재는 `isNewRecord`일 때만 하므로, 같은 실력으로 다시 클리어해도 돌아오지 못하고 자기 최고 기록을 넘어서야 한다). 그래서 반영에 실패하면 **적재 완료 마커를 내려** 다음 조회가 정본으로 폴백하고 재적재를 걸게 한다.
  - 그 응답의 순위도 캐시에서 읽지 않는다 — 마커가 내려간 상태이므로 `boss_rush_record`에서 계산한 값을 내려준다.
