...
- [6. 횡단 원장 — 재화·아이템 변동](#6-횡단-원장-재화아이템-변동)
...
이벤트 로그는 **전용 로거 카테고리 + 파일 sink**로 분리한다. 카테고리로 갈라 두면 운영 로그(콘솔)와 절대 섞이지 않는다.

```csharp
// Program.cs — 운영 로그(콘솔)는 로깅 규칙 §1 그대로 두고, 이벤트 로그만 파일로 분리한다.
builder.Logging.AddZLoggerRollingFile(options =>
{
    options.UseJsonFormatter();   // 평탄 JSON 1줄
    options.IncludeScopes = false;
});
// 카테고리 필터로 "TaskbarHero.EventLog" 로 시작하는 로거만 이 sink로 보낸다.
```

```csharp
/// <summary>액션 로그 1행을 방출한다. tag 가 곧 적재 테이블을 결정한다(4.3).</summary>
public void Action(string tag, long? uid, string? reqId, IEventFields fields)
{
    // fields 는 이벤트별 record 타입(컬럼과 1:1). snake_case 직렬화로 컬럼명을 맞춘다.
    _logger.ZLogInformation($"{EventLine.Create(tag, uid, reqId, fields):json}");
}
```

- 태그는 **`[CallerMemberName]`으로 자동 채우지 않고 상수로 명시**한다(`LogTags.StageClear`). 태그가 테이블을 결정하므로, 메서드 이름을 리팩터링하면 적재 대상이 바뀌는 결합은 만들지 않는다.
- 이벤트 필드는 **이벤트별 `record`**로 두어 컬럼과 1:1로 맞춘다. 익명 객체를 쓰면 컬럼이 조용히 어긋난다.
- 호출 지점은 **서비스 계층**이다(컨트롤러 금지 — 로깅 규칙 §4). 트랜잭션이 있는 이벤트는 **커밋 성공 이후** 방출한다(롤백된 사실을 로그에 남기지 않는다).
- 정확한 ZLogger API 시그니처(파일명 셀렉터·롤링 조건)는 구현 단계에서 확인한다 — 위 블록은 배치 위치를 보이기 위한 스케치다.
...
아래 규격을 **액션 로그 테이블 20개**(도메인 18 + 원장 2) 전부가 공유한다. **ERD(8.2~8.7)에는 공통 컬럼을 적지 않고 고유 컬럼만 그린다.**
...
3. **횡단 원장(6장)** — `currency_flow_logs`·`item_flow_logs`를 재화·아이템이 움직이는 모든 지점에 심는다. **경제 질문에 가장 먼저 답할 수 있게 되는 지점**이고 큐브·소모품·가방·룬·캐릭터 생성의 **유일한 기록**이라, 도메인 로그보다 먼저 한다.
