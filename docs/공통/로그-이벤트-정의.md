...
```yaml
  fluentd:
    ...
    volumes:
      ...
      - ./GameServer/logs/event:/var/log/taskbarhero/event:ro   # GameServer 가 쓰는 이벤트 JSON
...
- **수집 대상은 호스트 디렉터리 하나(`GameServer/logs/event`)를 바인드 마운트한다.** 서버를 콘솔(`dotnet run`·`watch-all.ps1`)로 띄우든 컨테이너로 띄우든 이벤트 로그가 **같은 폴더**에 모이도록, `gameserver` 서비스도 그 경로를 `/app/logs/event`(WORKDIR + `EventLog:Directory`)에 마운트한다. 그래서 fluentd는 실행 방식과 무관하게 이 한 디렉터리만 tail 한다.
