...
- [9. fluentd 설정](#9-fluentd-설정)
  - [9.1 입력 — 이벤트 파일 디렉터리를 걷는다](#91-입력-이벤트-파일-디렉터리를-걷는다)
  - [9.2 라우팅과 적재](#92-라우팅과-적재)
  - [9.3 버퍼 · 유실 정책](#93-버퍼-유실-정책)
  - [9.4 새 이벤트를 추가하는 절차](#94-새-이벤트를-추가하는-절차)
  - [9.5 전체 설정 파일](#95-전체-설정-파일)
  - [9.6 파이프라인이 흐르는지 확인하는 법](#96-파이프라인이-흐르는지-확인하는-법)
- [10. 볼륨 감각](#10-볼륨-감각)
- [11. 도입 순서](#11-도입-순서)
- [12. 미결 사항](#12-미결-사항)
- [13. Grafana 대시보드](#13-grafana-대시보드)
  - [13.1 프로비저닝 구조](#131-프로비저닝-구조)
  - [13.2 대시보드 다섯 장](#132-대시보드-다섯-장)
  - [13.3 집계 단위 변수와 추이 보기](#133-집계-단위-변수와-추이-보기)
...
- **대상 서버**: **`GameServer`와 `BatchServer`**(게임 도메인의 두 프로세스)다. 둘은 **같은 호스트 디렉터리**에 쓰므로 수집기 입장에서는 방출자가 하나인 것과 같다. `AccountServer`는 이벤트 로그를 방출하지 않는다 — 계정 도메인의 질문은 GameServer 로그와 지표로 나눠 답한다(5.1).
- **설계 원칙 다섯 가지**
...
flowchart LR
    subgraph APP["게임 서버(호스트 또는 컨테이너)"]
        ...
        A2["GameServer · BatchServer<br/>ZLogger"]
    end
    C["콘솔"]
...
- **fluentd는 1대다 — 역할도, 서버별 인스턴스도 나누지 않는다.** 방출자가 GameServer·BatchServer 둘이지만 **같은 디렉터리에 쓰므로 수집 대상은 여전히 파일 하나의 묶음**이라, 한 대로 두면 전달 홉이 없어 **지연·유실 지점·운영 대상이 각각 하나씩 줄어든다.**
...
이벤트 로그는 **전용 로거 카테고리 + 파일 sink**로 분리한다. 카테고리로 갈라 두면 운영 로그(콘솔)와 절대 섞이지 않는다. 구현은 `GameServer/Logging/`(`IEventLogger`·`EventLogger`·`EventFields`)에 있고 — GameServer와 BatchServer가 공유한다 — 로거 카테고리와 태그 상수는 `GameServer/Constants.cs`의 `Constants.EventLog`에 있다. `req_id`처럼 호스트마다 다른 값은 `IRequestIdAccessor`로 추상화해 각 호스트가 구현을 등록한다(GameServer=HttpContext.TraceIdentifier, BatchServer=항상 null).
...
{
    // 방출 실패는 여기서 흡수한다 — 호출자에게 올리지 않는다(아래 bullet).
    try
    {
...
private void Emit(string tag, long? uid, IEventFields fields, int? errorCode)
{
    ...
    // IRequestIdAccessor로 현재 요청에서 찾는다(요청 밖 = 배치면 없음).
...
- **방출 실패는 요청을 실패시키지 않는다.** 커밋 뒤에 부르는 자리라, 예외를 올리면 서비스의 공통 `catch`가 **이미 반영된 처리를 `ServerError`(500)로** 만든다. 그래서 `Action`이 실패를 흡수하고 **호출자에게 예외를 올리지 않으며**, 유실 사실만 **운영 로거로 Error 1줄** 남긴다(이벤트 카테고리로 내면 JSON 라인 사이에 섞여 적재가 깨진다 — 그래서 `EventLogger`는 로거를 둘 가진다). 레벨이 Error인 이유는 직렬화·sink 실패가 경합이 아니라 서버 결함이기 때문이다([로깅 규칙](로깅-규칙.md) §5).
...
- `batch.run`은 배치 종류별로 테이블을 나누지 않고 **`batch_key`로 구분**한다(`trade-expire`·`bossrush-season`·`mail-gc`·히스토리 3종 `history-online-user`·`history-hourly`·`history-daily`). 컬럼 구조가 배치마다 같기 때문이고, `server.start`/`stop`이 `phase` 컬럼으로 한 테이블인 것, 거래 결말 셋이 `outcome` 한 컬럼인 것(5.7)이 모두 같은 기준이다.
...
- **서버 구분 컬럼(`server`)을 두지 않는다.** 방출자가 GameServer·BatchServer 둘이지만 **어느 쪽이 냈는지로 갈라 볼 질문이 없다** — 배치가 내는 이벤트는 태그(`batch.run`·`history.*`)로 이미 구분되고, 나머지는 전부 GameServer가 낸다(8.1이 `tag` 컬럼을 두지 않는 것과 같은 이유). AccountServer가 이 체계에 합류하면 그때 추가한다.
...
- 일 단위 6종(`item_supply`·`stage_progress`·`equip_item`·`party_comp`·`skill_build`·`skill_invest`)은 `player_item`·`player_item_equipped`·`player_character`·`player_skill` 전체를 `GROUP BY` 하는 무거운 집계라 **하루 1회, 트래픽이 낮은 시간대**에 한 배치로 묶어 돌린다.

**배치는 주기별로 셋**이며, `batch.run`의 `batch_key`(5.11)가 곧 그 이름이다. 나눈 기준은 주기가 같은가가 아니라 **같은 시점의 스냅샷이어야 서로 나눠 볼 수 있는가**다.

| 배치(`batch_key`) | 주기 | 내보내는 태그 |
|---|---|---|
| `history-online-user` | 5분 | `history.online_user` |
| `history-hourly` | 1시간 | `history.currency_supply` · `history.trade_market` |
| `history-daily` | 1일(KST 05시) | 일 단위 6종 |

- **`history-hourly`가 둘을 함께 내는 이유.** 유통 총량과 호가는 짝으로 읽어야 뜻이 생긴다 — 총량이 늘면서 호가가 오르면 인플레이션이고, 총량이 그대로인데 호가만 오르면 품귀다. 배치를 나누면 두 스냅샷의 시각이 어긋나 그 비교가 흐려진다.
- **`online_count`는 "마지막 활동이 10분 이내인 계정 수"다.** 하트비트가 5분 주기라 창을 그보다 짧게 잡으면 정상 접속자가 밖으로 새고, 길게 잡으면 이미 나간 유저가 남아 동접이 부풀려진다 — **하트비트 주기의 2배**가 한 번 유실돼도 견디는 최소 여유다. 접속이 0이어도 방출한다(빈 구간이 "집계가 안 돈 것"과 구분되지 않으면 추이 그래프에 구멍이 생긴다).
- **`log_date`는 집계를 돌린 시점의 KST 날짜**이고 일 단위 6종이 그 값을 공유한다 — 자정을 걸쳐 실행돼도 한 스냅샷이 두 날짜로 갈라지지 않게 배치가 시작할 때 한 번만 읽는다.
- **`history-daily`는 폴링하지 않는다.** 다음 KST 05시까지 자고 그때 깨어난다 — 고정 24시간 간격으로 두면 실행 시각이 **서버 기동 시각에 끌려다녀** 재기동을 몇 번 하면 집계가 한낮으로 밀린다. 기동 직후에도 1회 도는데(주기 배치 공통 골격의 기본 동작), 자연 키 PK라 같은 날 다시 돌면 **덮어쓰는 것이 정상**이고 예정 시각에 서버가 내려가 있었더라도 그날 스냅샷이 비지 않는다.
- **`stage_progress`의 `stage_id`는 진행 시퀀스를 환산한 값이다.** 게임 DB(`game_player.max_stage_cleared`)가 들고 있는 값은 1~100의 선형 시퀀스라, 다른 스테이지 테이블과 같은 축(`stage_id`)으로 조인되려면 좌표 규약으로 바꿔야 한다. 아직 한 판도 깨지 못한 계정은 **`stage_id=0`으로 그대로 센다** — 가입 직후 이탈의 규모이기 때문이다.
- **`skill_build`의 두 코드는 오름차순**이라 같은 조합이 순서 차이로 갈라지지 않는다(파티와 달리 장착 순서에 의미가 없다). 1개만 장착한 캐릭터는 두 번째가 **0**이다 — `(코드, 코드)`로 두면 같은 스킬을 두 번 장착한 것으로 읽힌다.
- **`currency_supply`의 `mail_pending_amount`는 골드 행에만 붙인다.** 메일 첨부의 골드는 `reward_type=1` 하나로 표현되어 재화 코드를 따로 갖지 않기 때문이다(첨부의 `reward_code`는 0).
...
**실행 가능한 DDL 정본은 [`logdb-schema.sql`](logdb-schema.sql)이다** — 이 장의 테이블 31개(액션 21 + `unknown_event_logs` + 히스토리 9)와 하이퍼테이블·인덱스·압축·보존·연속 집계가 그 파일 하나에 있고, `docker-compose`의 `timescaledb` 서비스가 첫 기동 시 실행한다. 8장을 고치면 그 파일도 같은 작업에서 고친다.
...
erDiagram
    ...
    batch_run_logs {
        text batch_key "trade-expire / bossrush-season / mail-gc / history-*"
        int processed
        int skipped
        int failed
        int elapsed_ms
...
- `batch_run_logs`·`master_load_logs`·`server_lifecycle_logs`는 **`uid`·`req_id`가 없는 시스템 테이블**이다(8.1). `server` 구분 컬럼도 두지 않는다 — `batch_run_logs`는 BatchServer가, 나머지는 GameServer가 내며 태그로 이미 갈린다(5.11).
- `api_error_logs`만은 **`uid`가 NULL을 허용한다** — 인증 전 단계에서 터진 예외도 이 테이블에 오기 때문이다. 다른 유저 이벤트 테이블의 `uid`는 NOT NULL이다.
...
- **덮어쓰기는 DB가 흡수한다 — 수집기가 못 하기 때문이다.** `out_sql`(fluent-plugin-sql)은 순수 `INSERT`만 내고 `ON CONFLICT`를 낼 방법이 없어, 그대로 두면 두 번째 스냅샷이 유니크 위반으로 **통째로 버려진다**(실측: 일 배치가 하루에 두 번 돌자 일 단위 6종 368행 유실). 그래서 히스토리 9개 테이블에 **`BEFORE INSERT` 트리거**(`history_replace_row`)를 걸어 같은 자연 키의 기존 행을 지우고 새 행을 넣는다(= REPLACE). 서버 코드와 fluentd 설정은 이 문제를 몰라도 된다.
  - **액션 로그에는 걸지 않는다** — append-only이고, 같은 사건이 두 줄이면 그것 자체가 사실이다.
  - 키 컬럼과 **대상 하이퍼테이블 이름**을 트리거 인자로 넘긴다. TimescaleDB는 `BEFORE INSERT` 트리거를 **청크에 복제해** 실행하므로 `TG_TABLE_NAME`이 하이퍼테이블이 아니라 청크 이름이 된다 — 그 값으로 `DELETE`하면 다른 청크의 같은 키를 놓친다.
...
- **압축은 7일 지난 청크부터**, `segmentby`는 그 테이블에서 가장 자주 `GROUP BY` 하는 컬럼(`stage_clear_logs`·`stage_fail_logs`는 `stage_id`, `currency_flow_logs`는 `source`, `item_flow_logs`·`item_enhance_logs`·`trade_register_logs`는 `item_code`, `gacha_pull_item_logs`는 `gacha_code`, `trade_close_logs`는 `outcome`, `batch_run_logs`는 `batch_key`, `unknown_event_logs`는 `tag`, 나머지는 `uid`), `orderby`는 `timestamp DESC`. 최근 1주는 원본 그대로 두어 임시 분석이 빠르다.
  - **`master_load_logs`·`server_lifecycle_logs`는 `segmentby`를 비운다** — `uid`도 없고 `GROUP BY` 축도 없다(하루 몇 행이라 압축 이득도 없다).
  - **히스토리 테이블은 압축하지 않는다.** 자연 키 덮어쓰기가 정상 동작인 테이블이라 압축된 청크에 재적재가 걸리면 곤란하고, 볼륨도 하루 수백~수천 행에 그친다(7장).
...
- **`trade_close_logs`가 인덱스 3개인 유일한 예외다**(`uid`·`item_code`·`listing_id`) — 등록 행과의 조인 축을 포기하면 시세비·체결 시간을 낼 수 없다.
- **`api_error_logs`에는 `uid` 인덱스를 두지 않는다** — 시스템 테이블이라 최근 N건 조회가 전부다(8.6).
...
**모든 뷰를 `timescaledb.materialized_only = false`(실시간 집계)로 만든다.** 이게 없으면 뷰가 *구체화된 구간만* 돌려주는데, 갱신 정책이 `end_offset`(1시간) 앞을 남겨 두고 한 시간에 한 번만 도므로 **최근 로그가 대시보드에서 통째로 사라진다** — 실측으로 원본 `stage_clear_logs` 27행일 때 `stage_clear_daily`가 0행이었다. `false`로 두면 뷰가 "구체화된 부분 + 아직 안 굴린 꼬리의 실시간 집계"를 합쳐 돌려주므로 방금 쌓인 로그도 그대로 보인다. **TimescaleDB 2.13+의 기본값이 `true`라 반드시 명시한다.**
...
- **버킷은 `Asia/Seoul` 기준 1일이다**(`time_bucket('1 day', "timestamp", 'Asia/Seoul')`). 서버·컨테이너가 모두 KST로 도는데 UTC 버킷을 쓰면 "어제 골드 유입"이 09:00 경계로 잘려 운영 감각과 어긋난다. 뷰의 실제 컬럼 이름(`enter_count`·`gold_sum` 등)은 [`logdb-schema.sql`](logdb-schema.sql)이 정본이다.
...
**fluentd 1대, 설정 파일 하나(`fluent.conf`)로 끝낸다.** 입력 → 라우팅 → 적재가 한 프로세스 안에 있고, 방출자 둘(GameServer·BatchServer)이 같은 디렉터리에 쓰므로 입력도 하나다. **인스턴스 간 전달이 없으므로 `out_forward`는 쓰지 않는다.**

구현은 `fluentd/`(`Dockerfile` · `fluent.conf` · `conf.d/*.conf`)에 있고, 컨테이너 배선은 `docker-compose.yml`의 `fluentd` 서비스다.

```dockerfile
# fluentd/Dockerfile — 베이스 이미지 + 플러그인 두 개
FROM fluent/fluentd:v1.17-debian-1
USER root
RUN apt-get update -y     && apt-get install -y --no-install-recommends build-essential libpq-dev     && fluent-gem install --no-document fluent-plugin-rewrite-tag-filter     && fluent-gem install --no-document fluent-plugin-sql pg     && rm -rf /var/lib/apt/lists/*
# pos_file·버퍼 디렉터리를 미리 만들어 fluent 사용자에게 넘긴다 — 비어 있는 named volume은
# 이미지 쪽 디렉터리의 소유권을 물려받으므로, 이게 없으면 마운트 후 pos_file 쓰기가 막힌다.
RUN mkdir -p /var/log/fluentd/buf /var/log/taskbarhero/event     && chown -R fluent:fluent /var/log/fluentd
USER fluent
```

### 9.1 입력 — 이벤트 파일 디렉터리를 걷는다

```apacheconf
<source>
  @type tail
  path /var/log/taskbarhero/event/*.json   # GameServer·BatchServer 가 쓰는 이벤트 로그 디렉터리
  pos_file /var/log/fluentd/event.pos
  tag event.raw
  read_from_head true    # 새로 발견한 파일은 처음부터 읽는다(pos_file 이 중복을 막는다)
  refresh_interval 5s    # 날짜가 바뀌어 새 파일이 생겼을 때 기본 60초를 기다리지 않게
  <parse>
    @type json
    time_key timestamp
    time_type string
...
- **입력은 디렉터리 glob 하나다.** 방출자가 둘이어도 같은 디렉터리에 쓰므로 서버별 `<source>`가 필요 없다(파일명만 `event-*.json` / `event-batch-*.json`로 갈려 서로 붙잡지 않는다). 나중에 AccountServer를 합류시켜도 **같은 디렉터리에 쓰게 하면 이 블록을 고치지 않는다**(5.1·12장).
- `keep_time_key true`로 **`timestamp`를 레코드에 남긴다.** 적재되는 시각 컬럼은 fluentd가 읽은 시각이 아니라 **서버가 찍은 값**이어야 한다.
- **`read_from_head true`로 둔다.** 기본값(`false`)은 새로 발견한 파일을 끝에서부터 읽어, 이벤트 파일이 갈리는 순간(날짜·100MB 롤링)부터 fluentd가 그 파일을 발견하기까지 쓰인 라인이 사라진다. 이미 읽은 구간은 `pos_file`이 막으므로 중복은 생기지 않는다.
...
```apacheconf
# ── 입력: GameServer·BatchServer 가 쓰는 이벤트 JSON 파일(9.1) ─────────
<source>
  @type tail
...
`docker compose` 쪽 배선은 이 설정이 기대하는 마운트·환경변수만 맞추면 된다(현재 `docker-compose.yml`의 `fluentd` 서비스).

```yaml
  fluentd:
    build: ./fluentd
    image: taskbar-hero-fluentd
    container_name: taskbar-hero-fluentd
    restart: unless-stopped
    environment:
      TZ: Asia/Seoul
      LOGDB_PASSWORD: "${LOGDB_PASSWORD:-taskbar_hero_dev}"
    volumes:
      - ./fluentd/fluent.conf:/fluentd/etc/fluent.conf:ro
      - ./fluentd/conf.d:/fluentd/etc/conf.d:ro
      - ./GameServer/logs/event:/var/log/taskbarhero/event:ro   # GameServer·BatchServer 가 쓰는 이벤트 JSON
...
`timescaledb` 서비스는 `logdb` 데이터베이스와 `fluentd` 계정(컨테이너 수퍼유저)을 만들고, **첫 기동 시** `docs/공통/logdb-schema.sql`을 `/docker-entrypoint-initdb.d`에서 실행해 8장의 테이블·정책을 세운다. 헬스체크를 `pg_isready -h 127.0.0.1`(TCP)로 두는 이유는 MySQL과 같다 — init 스크립트를 돌리는 임시 서버는 유닉스 소켓만 열기 때문에, TCP를 보면 **스키마 생성이 끝난 뒤**에야 `healthy`가 되고 `fluentd`가 그 뒤에 뜬다.

- **수집 대상은 호스트 디렉터리 하나(`GameServer/logs/event`)를 바인드 마운트한다.** 서버를 콘솔(`dotnet run`·`server_up.py`)로 띄우든 컨테이너로 띄우든 이벤트 로그가 **같은 폴더**에 모이도록, `gameserver`·`batchserver` 서비스가 그 경로를 각각 `/app/logs/event`(WORKDIR + `EventLog:Directory`)에 마운트한다. 그래서 fluentd는 실행 방식과 무관하게 이 한 디렉터리만 tail 한다. 두 프로세스가 같은 폴더를 쓰지만 파일명 접두사가 달라(`event-*.json` / `event-batch-*.json`) 서로 같은 파일을 잡지 않는다.
- **`pos_file`과 버퍼는 볼륨에 둔다.** 컨테이너를 다시 만들 때 위치를 잃으면 읽은 구간을 다시 읽어 중복 적재가 된다(9.3).
- **`LOGDB_PASSWORD`는 기본값을 둔다.** 컴포즈의 변수 보간은 파일을 읽는 시점에 일어나므로, 필수(`:?`)로 두면 `docker compose up -d mysql` 처럼 다른 서비스만 띄우는 명령까지 막힌다. 값은 `.env`로 덮어쓴다.
- **스키마를 고쳐 다시 적용할 때는 볼륨을 비운다.** init 스크립트는 데이터 볼륨이 비어 있을 때만 돌기 때문에, `docker compose down -v` 후 재기동이 가장 확실하다(누적 로그를 지우고 싶지 않으면 변경분만 `psql`로 직접 적용한다).
- **`out_sql`은 기동 시 `logdb`에 접속한다.** 그래서 `logdb`가 죽어 있으면 fluentd 컨테이너가 재시작을 반복한다 — 그 사이의 라인은 서버가 쓴 JSON 파일에 남고, 정상화되면 `pos_file` 위치부터 이어 읽으므로 유실되지 않는다(9.3).
- 로그 드라이버로 갈아탈 때는 `<source>`만 `@type forward`로 바꾼다(9.1·3장).
---

### 9.6 파이프라인이 흐르는지 확인하는 법

대시보드에 그래프가 그려지면 **적재까지는 됐다**는 뜻이지만, 그래프가 비었을 때 *어디가* 막혔는지는 알려 주지 않는다. 파이프라인은 네 구간이고 각 구간이 서로 다른 신호를 남긴다.

| 구간 | 무엇을 보나 | 어디서 |
|---|---|---|
| 서버가 쓰고 있나 | 이벤트 파일 크기·마지막 라인의 `timestamp` | `GameServer/logs/event/*.json` |
| 수집이 따라잡나 | `pos_file` 오프셋 vs 파일 크기 = **아직 안 읽은 바이트** | fluentd 컨테이너의 `/var/log/fluentd/event.pos` |
| 전송이 밀리나 | 버퍼 큐 길이 · `retry_count` · `rollback_count` | `in_monitor_agent` — `http://127.0.0.1:24220/api/plugins.json` |
| 적재가 되고 있나 | 최신 행 시각과 지금의 차 · `unknown_event_logs` | `logdb` |

- **`in_monitor_agent`를 켜 두는 이유.** 버퍼가 쌓이는지·재시도가 도는지는 컨테이너 안 디렉터리를 뒤져야 알 수 있고, `pos_file`은 16진 오프셋이라 눈으로 읽기 어렵다. 표준 엔드포인트 하나로 내면 사람도 스크립트도 같은 값을 본다. 호스트 루프백에만 바인딩한다(운영 정보라 외부에 열 이유가 없다).
- **네 구간을 한 번에 보는 것은 [`pipeline-status.ps1`](../../pipeline-status.ps1)이다.** `-Watch`를 주면 주기적으로 갱신한다. 판정 기준은 **미수집 바이트 0 · 대기 청크 0 · 재시도 0 · 끝단 지연 < flush 주기(10초)** 다.
- **막힘은 구간마다 다르게 보인다.** logdb가 죽으면 *수집은 계속되고*(`미수집 0`) **전송 버퍼만 쌓인다** — 서버가 쓴 파일이 1차 버퍼이므로 그동안 라인이 사라지지 않는다(9.3). 반대로 서버가 죽으면 파일의 마지막 쓰기 시각이 멈추고 버퍼는 비어 있다. 즉 **"대기 청크 > 0 + 미수집 0"이면 DB 쪽, "마지막 쓰기가 오래됨 + 버퍼 비어 있음"이면 서버 쪽**이다.
- **`unknown_event_logs`가 0이 아니면 카탈로그 누락**이다 — 서버가 낸 태그에 `<table>` 매핑이 없다는 뜻이라, 9.4의 절차를 빠뜨린 것이다.
...
`logdb`에 쌓인 로그를 읽는 화면이다. **UI에서 손으로 만들지 않고 저장소의 파일이 정본**이며, 그라파나 컨테이너가 기동 시 프로비저닝으로 읽어 등록한다 — 대시보드가 어떤 질문에 답하는지가 이 문서 5~8장과 1:1로 대응해야 하기 때문이다.
...
grafana/
├── provisioning/
│   ├── datasources/logdb.yaml     # logdb 데이터소스(uid: logdb) — 대시보드가 이 uid를 가리킨다
│   └── dashboards/logdb.yaml      # 대시보드 공급자(폴더 「게임 로그」, 30초마다 파일 재확인)
└── dashboards/*.json              # 대시보드 정본 5장
```

- **데이터소스 `uid`는 `logdb`로 고정**한다. 대시보드 JSON의 모든 패널이 이 uid를 참조하므로, UI에서 데이터소스를 다시 만들면(uid가 새로 생긴다) 전부 깨진다.
- 접속은 compose 네트워크의 `timescaledb:5432`이고 **비밀번호는 파일에 적지 않는다** — 컨테이너 환경변수 `LOGDB_PASSWORD`를 프로비저닝 YAML이 `${LOGDB_PASSWORD}`로 보간한다([로깅 규칙](로깅-규칙.md) §7과 같은 기준).
- `jsonData.timescaledb: true`로 둬야 쿼리 편집기가 `time_bucket`을 인식하고 `$__timeGroup` 매크로가 그것으로 번역된다.
- **UI 편집은 허용하되 정본은 JSON이다**(`allowUiUpdates: true`). 편집한 것을 남기려면 대시보드를 JSON으로 내보내 `grafana/dashboards/`에 덮어쓴다. 그러지 않으면 컨테이너 재기동 때 파일 내용으로 되돌아간다.
- 모든 대시보드는 **`server_lifecycle_logs`를 주석(annotation)으로 깔아 둔다** — 수치가 튄 시점이 배포 때문인지 한눈에 가른다(5.11).

### 13.2 대시보드 다섯 장

ERD 구분(8.2~8.6)을 그대로 화면 경계로 삼았다. 한 화면이 한 도메인의 질문을 닫는다.

| 파일 · uid | 화면 | 답하는 질문 | 주 원본 |
|---|---|---|---|
| `logdb-session.json` | 1. 접속 · 리텐션 | 활성 유저(DAU/WAU)·신규 유입·복귀 주기·D+1/D+7 유지율·첫 직업 분포 | `session_user_daily` · `save_load_logs` · `player_create_logs` |
| `logdb-stage.json` | 2. 스테이지 · 성장 | 실질 난이도(실패/진입)·이탈(진입−클리어−실패)·리밸런싱 후보 50개·보상 유입·레벨업 출처·방치 상한 도달률 | `stage_*_daily` · `character_levelup_logs` · `offline_claim_logs` |
| `logdb-economy.json` | 3. 경제 · 원장 | 골드 유입/유출/소각과 순증·유입원·유출원 Top·아이템 순증 상위·강화 단계별 골드 벽 | `currency_flow_daily` · `item_flow_daily` · `item_enhance_logs` |
| `logdb-gacha-trade.json` | 4. 가챠 · 거래소 · 메일 · 출석 | 배너별 등급 실측 확률·천장 의존도·미체결률·품목별 시세와 체결 시간·수수료 소각·메일 발급 대비 수령(우편함 부채)·출석 일차 이탈 | `gacha_grade_daily` · `trade_*_logs` · `mail_issue_logs` · `attendance_claim_logs` |
| `logdb-bossrush-system.json` | 5. 보스러시 · 시스템 | 완주율·라운드 병목·시즌 기록 분포 / 미처리 예외 추이와 최근 목록·배치 처리량과 실패·마스터 적재·**카탈로그 누락(`unknown_event_logs`)** | `bossrush_*_logs` · `api_error_logs` · `batch_run_logs` · `unknown_event_logs` |
| `logdb-history.json` | 6. 히스토리 — 총량 · 상태 스냅샷 | 동시 접속·**유통 골드 총량과 우편함 부채**·계정당 평균 보유·품목 시세 추이와 최신 호가·유통량 상위 아이템·진행도 분포·착용 장비/파티 조합/스킬 빌드·투자 분포 | `*_history` 9종(7장) |

- **패널은 연속 집계를 먼저 쓴다**(8.10). 원본을 직접 읽는 패널은 집계 뷰가 없는 도메인(거래소·메일·출석·보스러시·시스템·강화)뿐이다.
- **`unknown_event_logs` 패널에 행이 보이면 그 자체가 이상 신호**다 — 방출 코드와 이 문서의 카탈로그(5·7장)가 어긋났다는 뜻이다(9.5).
- **히스토리는 6번 화면으로 따로 뗐다.** 액션 로그가 "무슨 일이 일어났나"라면 히스토리는 "지금 얼마나 쌓여 있나"이고(2장), 시간축 컬럼도 다르다(5분·1시간 주기는 `timestamp`, 1일 주기는 `log_date`). 총량은 **재고**라 구간 안에서 더하면 안 되므로 `last(값, "timestamp")`로 그 구간 마지막 스냅샷을 고른다 — 합계 패널과 섞어 두면 이 차이가 묻힌다.
- **1일 주기 히스토리 6종은 집계 단위 변수와 무관하게 하루 한 점**이고, 상태 스냅샷 4종(장착·파티·스킬)은 **선택 구간의 마지막 날짜 한 장만** 보여 준다(`log_date = (SELECT max(log_date) ...)`). 추이가 아니라 분포를 보는 표다.

### 13.3 집계 단위 변수와 추이 보기

모든 추이 패널은 **기간(상단 시간 선택)과 집계 단위(`$bucket` 변수)** 두 손잡이로 본다.

- `$bucket`은 `5 minutes` · `1 hour` · `6 hours` · `1 day` · `7 days`다. 원본 테이블 패널은 고른 값을 그대로 `time_bucket(INTERVAL '$bucket', "timestamp", 'Asia/Seoul')`에 넣는다. 기본값은 화면마다 다르다 — 경제·스테이지·가챠는 `1 day`, 시스템(5번)과 히스토리(6번)는 `1 hour`다(동접·예외는 하루로 뭉치면 신호가 사라진다).
- **연속 집계 패널은 1일이 하한**이다(뷰 자체가 일 버킷이라 그보다 잘게 나눌 수 없다). 그래서 `time_bucket(GREATEST(INTERVAL '$bucket', INTERVAL '1 day'), bucket, 'Asia/Seoul')`으로 바닥을 깐다 — 1시간을 골라도 일 단위로 남는다.
- **버킷은 언제나 `Asia/Seoul` 기준**이다(8.10). `$__timeGroup` 대신 `time_bucket`에 시간대를 직접 넘기는 이유가 이것이다 — 매크로는 UTC 경계로 잘라 "어제"가 09:00에 끊긴다.
- 「활성 유저」 패널은 `count(DISTINCT uid)`라 **단위를 1일로 두면 DAU, 7일로 두면 WAU**가 된다. 합계·건수 패널은 단위를 늘리면 그대로 기간 합계가 된다.
