...
| Redis | `GameServer` | **보스러시 랭킹 리더보드**(`rank:bossrush:{seasonId}`, Sorted Set, 점수 = `clearMs × 10^7 + (recordedAt − season.start_at)`) — 시즌 순위 조회 전용 **파생 인덱스**. 정본은 MySQL `boss_rush_record`이며 ZADD는 커밋 이후에만 한다. `ZRANK`(내 순위)·`ZRANGE`(페이지)가 O(log N + M)이라 **전체 등재 유저를 순위 상한 없이 페이징**할 수 있다([보스러시 기획서](../세부/boss-rush-기획서.md) 4.3·6.3) |
| Redis | `GameServer` | **리더보드 적재 완료 마커**(`rank:bossrush:{seasonId}:ready`, String) — **이 키가 있을 때만 조회가 리더보드를 신뢰**하고, 없으면 MySQL 정렬 조회로 폴백하며 자동 재적재를 건다. 리더보드 키의 존재로는 판단할 수 없기 때문에 따로 둔다 — 없는 키의 `ZCARD`는 0을 돌려줘 캐시 부재와 "등재 0명"이 구별되지 않고, 클리어 보고의 ZADD가 키를 새로 만들어 **한 명만 든 리더보드**가 정상처럼 보인다. 워밍업이 전량 적재를 마쳤을 때만 세워지고, 재적재 시작·ZADD 실패 시 내려간다. 시즌 종료 시 리더보드와 같은 7일 TTL(4.3·6.3) |
| Redis | `GameServer` | **리더보드 재적재 락**(`rank:bossrush:{seasonId}:rebuilding`, String, `SET NX` + TTL 2분) — 마커 부재를 감지한 게임 API 인스턴스가 **N대 동시에 재적재하지 않도록** 하나만 통과시킨다. 적재 경로는 관리 API(`POST /api/admin/boss-rush/rank/warmup`, 부트스트랩 스크립트 `server_up_with_docker.py`가 기동 시 호출)와 조회가 거는 자동 재적재 둘이다(4.3·6.3) |
...
- **재화 행 생성 주체**: **세이브 생성**(`create-character` 최초 호출)이 골드 재화 행(`row_type=2`·`item_code=1`)을 **잔액 0으로 미리 만든다**. 그 뒤로는 어떤 경로도 재화 행을 새로 만들거나 지우지 않으며, 골드를 주고받는 모든 경로는 이미 있는 행의 `quantity`만 더하고 뺀다. 지급이 "행이 없으면 만든다"를 판단하면 잔액 0인 계정에 지급 두 건이 동시에 들어올 때 양쪽이 모두 '행 없음'을 보고 각자 만들어 **재화 행이 둘로 갈라지기** 때문이다([세이브 데이터 기획서](../세부/save-data-기획서.md) 5.3).
- **잔액 가감 방식**: 읽어서 계산한 값을 쓰지 않고 `quantity = quantity ± N`으로 **DB가 직접 계산**하며, 차감은 `quantity >= N`을 같은 문장의 조건으로 둔다. 같은 계정의 요청 둘이 겹쳐도 한쪽 변경이 다른 쪽에 덮이지 않는다. 갱신은 `player_item_id`(기본키)로 걸어 **그 한 행만 잠근다** — 계정·종류 조건으로 바로 갱신하면 맞는 인덱스가 없어 그 계정의 아이템 행까지 훑으며 잠그고, 잠그는 순서가 엇갈리면 교착이 난다.
- **아이템 수량·배치 갱신 방식**: 스택 병합·재료 차감처럼 계산이 필요한 값은 대상 행을 **잠금 조회**(`SELECT ... FOR UPDATE`)로 읽어 최신 수량을 본 뒤, **읽은 수량 그대로일 때만** 갱신한다. 잠금 없는 조회는 트랜잭션이 처음 읽은 시점의 스냅샷을 계속 보므로 그 사이 커밋된 변경이 보이지 않는다 — 그 값으로 계산하면 조건이 매번 어긋난다. 빈 칸 배정도 같은 이유로 점유 칸을 잠금 조회로 읽으며, 그래도 겹치면 `uq_item_slot`이 막고 서버가 트랜잭션을 다시 실행해 다음 칸을 받는다.
...
- [`logdb-schema.sql`](logdb-schema.sql) — 그 로그 테이블 31개의 실행 가능한 DDL(하이퍼테이블·압축·보존·연속 집계 포함)
