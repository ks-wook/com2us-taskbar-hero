...
| 이름 | 값 | 의미 |
|---|---|---|
| ServerError | 11001 | 특정 도메인에 속하지 않는 서버 내부 오류. **서비스가 공개 메서드에서 잡은 예외**와 **전역 예외 처리기가 받은 미처리 예외**를 모두 이 코드로 일반화(내부 정보 미노출)해 HTTP 500으로 응답한다. 어느 경로든 예외는 Error로 1회 로깅된다. |
| CurrencyRowMissing | 11002 | 재화 행(`player_item`의 `row_type=2`)이 없어 차감하지 못했다. **정상 경로로는 발생하지 않는다** — 재화 행은 세이브 생성 시 잔액 0으로 함께 만들고 코드 어디서도 지우지 않으므로, DB를 직접 손댄 경우에만 성립한다. 잔액 부족(`InsufficientCurrency` 4005)으로 응답하면 서버 결함이 사용자 실수로 안내되고 기록도 남지 않아 구분한다. Error로 1회 로깅하고 HTTP 500으로 응답한다. 골드를 차감하는 모든 경로(가챠 뽑기·룬 강화·장비 강화·가방 확장·큐브 제작·거래 구매·캐릭터 생성)가 공유한다. |
