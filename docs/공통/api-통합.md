...
- **호출 주체는 서버 밖의 부트스트랩 스크립트**(`python server_up_with_docker.py`)다. 컨테이너·서버 기동을 확인한 뒤 이 엔드포인트를 한 번 호출한다 — **서버는 랭킹 캐시를 스스로 적재하지 않는다.** 예전에는 시즌 정산 배치가 Redis 리더 락을 쥔 채 매 주기 앞단에서 이 일을 했으나, 적재 시점이 배치 주기에 묶여 보이지 않았고 호출자가 스크립트 하나로 정해지면서 중복 재구축을 막을 분산 락도 필요 없어졌다.
