...
| `POST /api/game/trade/register` | 판매 등록(에스크로) | `{ itemId, price }` | `listingId`, `itemCode`, `price`, `inventoryDelta` | `ItemNotFound(4001)`, `ItemEquipped(4007)`, `TradeNotSellable(7002)`, `TradePriceOutOfRange(7006)`, `TradeListingLimitExceeded(7007)`, `TradeBusy(7008)`, `InvalidSaveData(2002)` |
...
- 동시 구매 경합은 **MySQL 조건부 갱신**(`status=1`일 때만 전이)의 행 잠금으로 직렬화한다 — 뒤에 온 요청은 0행을 받아 `TradeAlreadyClosed(7005)`가 되며 복제·이중 판매가 불가하다. 취소·만료 배치도 같은 방식이고 **구매·취소·만료에는 Redis 락을 쓰지 않는다**. `TradeBusy(7008)`는 **판매 등록**의 판매자 단위 락 경합에서만 나온다.
