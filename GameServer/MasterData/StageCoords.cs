...
///   Normal(난이도1)에서 Act1~5(각 3스테이지)을 모두 깨면 Hard(난이도2)가 열린다.
///   sequence = (difficulty-1)*15 + (act-1)*3 + stage  → 1..30
///   Act/난이도 전진 순서(난이도-바깥)는 stage-battle 기획서 8장에서 확정된 채택안이다.
///   최종 스테이지(시퀀스 30) 클리어 후에는 전진처가 없어 그 스테이지에 머문다(기획서 8장 확정).
/// </summary>
public static class StageCoords
{
    ...
    public const int BossStage = 3;               // 각 Act·난이도의 3스테이지가 보스
    public const int TotalStages = Acts * Difficulties * StagesPerAct; // 30
    ...
    /// <summary>진행 시퀀스(1..30). 좌표가 유효하지 않으면 0.</summary>
    ...
    /// <summary>시퀀스(1..30)를 좌표로 디코딩. 범위 밖이면 false.</summary>
