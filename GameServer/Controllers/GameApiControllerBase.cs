...
public abstract class GameApiControllerBase : ControllerBase
{
    ...
    private static int HttpStatus(ErrorCode code) => code switch
    {
        ...
        // 출석: 오늘자 중복 수령(동시 요청 경합 포함)·이번달 사다리 소진은 409.
        ErrorCode.AttendanceAlreadyClaimed => StatusCodes.Status409Conflict,
        ErrorCode.AttendanceAllClaimed => StatusCodes.Status409Conflict,
    ...
    private static string ErrorMessage(ErrorCode code) => code switch
    {
        ...
        // 거래소
        ErrorCode.TradeListingNotFound => "Trade listing not found",
        ErrorCode.TradeNotSellable => "Item is not sellable",
        ErrorCode.TradeNotOwner => "Not the listing owner",
        ErrorCode.TradeSelfPurchase => "Cannot buy your own listing",
        ...
        ErrorCode.AttendanceAllClaimed => "All attendance rewards claimed this month",
