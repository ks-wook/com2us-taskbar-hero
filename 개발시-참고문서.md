...
- [11. 사운드 리소스 생성 및 적용](#11-사운드-리소스-생성-및-적용)
- [12. 게임 빌드 방법](#12-게임-빌드-방법)
...
| **Redis** | `redis:8.2-alpine` | `36379` → `6379` | 인증 토큰 · 배치 리더 락 · 보스러시 랭킹 캐시 |
...
BGM · 효과음도 외주 · 구매 없이 **텍스트 프롬프트로 생성**했다. 아이템 아이콘과 다른 점은 결과를 코드로
그릴 수 없다는 것이다(생성 모델을 부른다). 그래서 **무엇을 만들지와 어디서 재생할지를 문서로 먼저 확정**하고,
생성·검증·배선을 그 문서에 맞춰 돌렸다.

| 구성 요소 | 위치 | 하는 일 |
|---|---|---|
| 정본 문서 | [`docs/세부/사운드-리소스-정의.md`](docs/세부/사운드-리소스-정의.md) | ID · 사용 위치 · **트리거 지점** · 톤 · 길이 · 생성여부 · 클라적용 |
| 생성 도구 | `tools/poe-sfx/generate_sound.py` | 오디오 생성 API 호출 · 저장 · **길이 검증** |
| 에디터 도구 | `com2us-taskbar-hero-client/Assets/Editor/SoundDatabaseBuilder.cs` | 임포트 설정 교정 + 파일명 ↔ `SoundId` 매핑 빌드 |
| 재생 계층 | `Assets/Scripts/Managers/SoundManager.cs` 외 | BGM 2채널 크로스페이드 · SFX 풀 · 볼륨 3채널 |

### 11.1 목록을 먼저 확정한다

정의서에 **ID 하나당 한 줄**로 사용 위치 · 트리거 지점 · 톤 · 길이를 적어 두고, 그 표를 보면서 생성한다.
표의 마지막 두 칼럼이 현황판이다 — **생성여부**(파일이 있는가)와 **클라적용**(실제로 재생되는가)을 따로 센다.

- 폴더는 **카테고리까지만** 나누고 그 안에서 더 쪼개지 않는다. **파일명이 곧 식별자**이기 때문이다.
- 접두사로 종류를 구분한다 — `bgm_`(배경음) · `jingle_`(연출 징글) · `sfx_`(효과음) · `amb_`(앰비언트 루프).
- 현재 **생성 75 / 75 · 클라 적용 73 / 75**. 빠진 2종은 이유가 적혀 있다 — 툴팁은 **의도적 무음**(hover로 뜨는
  UI라 아이템 칸을 지나갈 때마다 울리면 거슬린다), 보스러시 시간 경고는 **제한 시간 규칙이 없어져 트리거가 없다.**



### 11.2 실사용 테스트 — UI 버튼 클릭음을 생성하고 적용까지

9.5·10.4와 같은 방식으로 **버튼 클릭음(`sfx_ui_click`)을 새로 생성해 실제 재생까지** 확인했다.
프롬프트는 지어내지 않고 **정의서 부록에 적어 둔 그 문장을 그대로** 썼다 — 정의서가 정본이라는 것이 이 뜻이다.

**① 생성**

```bash
python tools/poe-sfx/generate_sound.py \
  "Short crisp retro game UI button click, small wooden tick with a subtle chiptune blip, dry and clean, single hit, no music, no speech" \
  "<임시 경로>/sfx_ui_click_new.mp3" --duration 1
```

```
저장 완료: …/sfx_ui_click_new.mp3  (33,063 bytes)
검증: 길이 1.07초, 64/128 kbps, joint-stereo/stereo
```

- 도구가 저장 직후 **MP3 프레임을 훑어 길이를 재고** 요청(1초)의 2배를 넘지 않았음을 확인했다 — 낭독(TTS)으로
  새어 든 결과가 아니라는 뜻이다(11.1의 함정).
- **바로 덮어쓰지 않고** 임시 경로에 받아 검증한 뒤 교체했다. 이전 파일은 따로 보관했다(같은 1.07초, 16,971 bytes).

**② 적용 — 파일을 넣고 에디터 도구를 한 번 실행**

`Assets/Sound/SFX/UI/sfx_ui_click.mp3`로 복사한 뒤 Unity 메뉴 **`TaskbarHero/Sound/사운드 DB 빌드`** 를 실행했다.

```
클립 임포트: True · 길이 1.071초 · 스테레오 44.1kHz
임포트 설정: loadType=DecompressOnLoad (효과음 규약 · §11.1)
사운드 DB 빌드: True → SoundDatabase.asset 이 새 클립 GUID 를 참조
```

파일명이 곧 ID이므로 **코드·에셋을 손으로 고칠 일이 없다** — `sfx_ui_click.mp3` → `SoundId.UiClick`(열거값 11) →
`SoundDatabase` 항목 `id: 11`이 새 GUID로 갱신됐다.

**③ 재생 확인 — 플레이 모드에서 실제로 소리가 나는지**

```
UiClick → 클립 'sfx_ui_click'                  (SoundManager 가 들고 있는 SoundDatabase 로 해석)
PlaySfx(UiClick) 직후 AudioSource 13개 중 2개 재생 중   (BGM 1 + SFX 풀 1 — 클릭음은 PlayOneShot 이라 .clip 이 비어 있다)
씬의 UiClickSoundTag 7개                        (UiClickSound 가 버튼을 스캔해 클릭음을 붙인 결과)
```

- `AudioSource 13개` = **BGM 2채널 + 앰비언트 + SFX 풀 10**으로, 11.1에 적어 둔 재생 계층 구성과 그대로 맞는다.
- 확인이 끝난 뒤 플레이 모드는 종료했다(`isPlaying=False`).

> **사운드는 아이템·몬스터와 달리 서버가 개입하지 않는다** — 마스터 데이터가 아니라 클라이언트 에셋이라
> DB·번들·서버 재기동이 전부 필요 없고, **파일 교체 + 사운드 DB 빌드**로 끝난다. 반대로 정의서의 트리거 지점을
> 새로 늘리는 일(= 새 ID 추가)은 `SoundId`와 재생 지점 코드를 함께 고쳐야 하므로 클라이언트 작업이다.
---
