...
목표와 기준 (값 문서 §9.1 — 이 파일의 TARGET_SEC · ACT_STANDARD · ACT_FLOOR가 그 사본이다)
  ...
  · 긴장도   : 기준 파티의 **최저 체력**(전투 중 떨어진 최저값)이 판정 지표다. 난이도를 만드는 것은
                stage_spawn.monster_level 이고, 레벨이 지역 안에서 3스테이지마다 오르므로
                **한 지역 안에서 스테이지가 오를수록 좁아진다**(3지역 예: 91% → 73% → 46%).
...
#   난이도는 stage_spawn.monster_level 이 만들고, 레벨은 지역 안에서 3스테이지마다 오르므로
#   **지역 안에서는 스테이지가 오를수록 좁아진다**(예: 3지역 91% → 73% → 46%). 아래 값은
#   각 지역에서 가장 좁은 지점(9스테이지)과 보스 스테이지의 실측이다.
#   2지역이 가장 헐거운 것은 그 지역에서 인원이 2명으로 늘고 장비·강화·스킬 투자가 처음
#   들어오기 때문이고, 5지역이 가장 좁은 것은 인원이 더 늘지 않는데 레벨은 계속 오르기 때문이다.
TENSION_S9 = {1: 0.46, 2: 0.68, 3: 0.46, 4: 0.61, 5: 0.36}
TENSION_BOSS = {1: 0.52, 2: 0.43, 3: 0.47, 4: 0.57, 5: 0.53}
