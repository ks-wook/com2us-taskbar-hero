...
스테이지 진입·클리어(서버 권위 보상 산출). 모든 요청은 `GameAuthMiddleware` 인증을 거친다. 저장소: MySQL `taskbar_hero_game` + `MasterDataProvider`(stage/reward/level/item).

> **공통 인증**: 첫 단계는 `GameAuthMiddleware`의 Redis 토큰 대조(실패 시 401). 아래에서는 인증 통과 이후를 중심으로 표기한다.
...
sequenceDiagram
    ...
    C->>MW: POST /stage/enter { userId, token, data:{ act, difficulty, stage } }
    ...
    alt 마스터 미로드 / 스테이지 없음
        ...
        Repo->>DB: SELECT game_player(진행도)
        alt 세이브 없음
            ...
            alt 도달 불가(잠김)
                ...
                Repo->>DB: UPDATE game_player(현재 진입 스테이지)
...
sequenceDiagram
    ...
    C->>MW: POST /stage/clear { userId, token, data:{ act, difficulty, stage } }
    MW-->>Ctrl: 인증 통과(userId)
    ...
    alt 스테이지·보상 정의 없음
        ...
        Repo->>DB: 진입 스테이지 재검증
        Repo->>DB: 골드 지급(재화 행 upsert)
        Repo->>DB: 3캐릭터 경험치 지급 + 레벨 재계산(level_master)
        Repo->>DB: 전리품 적재(스택/용량 규칙)
        Repo->>DB: 진행도 갱신(프런티어면 다음 스테이지 전진)
        alt 미진입 / 용량 초과
