...
- **Redis 인스턴스**: 로컬 개발용 Redis는 **docker-compose의 `redis` 서비스**(`redis:8.2-alpine`, 컨테이너명 `taskbar-hero-redis`, **호스트 `127.0.0.1:36379` → 컨테이너 `6379`**, AOF 활성)를 기본으로 쓴다 — MySQL과 함께 `docker compose up -d`로 함께 뜬다. `redis-cli`는 `docker exec taskbar-hero-redis redis-cli ...`로 호출한다.
  - 저장소 내 Windows 바이너리 `Redis-8.8.0-Windows-x64-cygwin-with-Service/`(`start.bat` 또는 `redis-server.exe redis.conf`)도 그대로 쓸 수 있다. 컨테이너를 **호스트 36379**로 옮겼으므로 바이너리(6379)와 포트가 겹치지 않지만, 서버가 보는 Redis는 `appsettings`의 `Redis:ConnectionString` 하나이므로 **둘을 동시에 띄우면 어느 쪽에 쓰는지 헷갈린다** — 하나만 쓴다.
...
- **상수는 예외 없이 `Constants.cs`에 선언**: 매직 넘버·고정 문자열·불변 값(`const`, 그리고 숫자·문자열·`TimeSpan` 같은 값을 담은 `static readonly`)은 **`Constants.cs`**에만 선언한다(선언 파일은 아래 「상수 파일이 어디에 있나」를 따른다). 서비스·컨트롤러·리포지토리·미들웨어·배치 클래스 안에는 **private 상수라도 두지 않는다** — 같은 성격의 값이 여러 파일에 흩어지면 어디에 있는지 찾을 수 없고, 두 곳에서 각자 정의한 값이 조용히 어긋난다.
  ...
  - **상수 파일이 어디에 있나 — 게임 규칙과 프로세스 운영 설정을 나눈다.**

    | 파일 | 담는 값 | 판단 기준 |
    |---|---|---|
    | `GameServer/Constants.cs` | **게임 규칙·계약**(재화·스테이지·보스러시·거래·메일 보관 기간·메일 템플릿 코드·이벤트 로그 태그·Redis 키 등). `GameServer`·`BatchServer`가 함께 본다. | 게임 API가 읽거나, 기획서가 정하는 값 |
    | `BatchServer/BatchSettingConstants.cs` | **BatchServer 전용 운영 설정**(배치별 발화 시각·간격·1회 처리 상한·`appsettings` 키). | 게임 API가 하나도 읽지 않고, 배치 운영 사정으로만 바뀌는 값 |
    | `AccountServer/Constants.cs` | 계정 서버 전용 값(아직 파일 없음 — 상수가 처음 필요해지면 만든다). | |

    배치가 쓰더라도 **게임 규칙이면 `Constants.cs`에 남긴다**(예: 메일 보관 기간 `Constants.Mail.RetentionSeconds`, 반송 템플릿 `Constants.MailTemplate.TradeReturn`, 히스토리 로그 태그 `Constants.EventLog.Tags.History.*` — 로그 태그는 `docs/공통/로그-이벤트-정의.md`와 1:1인 카탈로그라 쪼개지 않는다). 반대로 발화 시각·처리 상한처럼 **그 프로세스를 어떻게 굴릴지**를 정하는 값은 배치 파일로 간다.
    `BatchSettingConstants.cs`는 파일 상단에 **여섯 배치가 각각 몇 시에 도는지 요약표**를 둔다 — 새 배치를 넣을 때 이 표를 보고 무거운 집계끼리 시각이 겹치지 않게 정한다.
  - 새 상수를 만들거나 기존 상수를 옮길 때 이 규칙을 매번 확인한다. 상수가 필요해지면 **선언 위치를 고민하지 않고 곧바로 위 표의 해당 파일**에 넣는다.
...
- **어시스턴트는 서버를 포어그라운드로만 띄운다**: 서버(`GameServer`·`AccountServer`·`BatchServer`)를 어시스턴트가 띄울 수 있으나, **반드시 사용자가 볼 수 있는 별도 콘솔 창(포어그라운드)** 으로만 띄운다. **숨김·백그라운드 기동은 금지**한다 — `Start-Process -WindowStyle Hidden`·`Start-Job`·harness `run_in_background`·`&`·`nohup` 등. 숨겨진 서버는 세션이 끝나도 남아 `bin/` 파일을 잠그고(MSB3021/MSB3027), 다음 테스트가 **옛 바이너리를 검증**하게 만든다.
  - 기동 명령(창이 뜨고 로그가 사용자에게 그대로 보인다):
    ...
    Start-Process powershell -ArgumentList '-NoExit','-Command','dotnet run --project BatchServer'
    ```
...
- **테스트 전후 기존 서버 강제 종료**: 테스트를 시작하기 전에 **이미 떠 있는 서버가 있으면 확인 없이 강제 종료한다.** 끝난 뒤에도 잔여 프로세스가 0인지 반드시 확인한다.
  - 종료는 **앱 프로세스와 `dotnet run` 래퍼를 모두** 잡는다. 래퍼가 살아 있으면 자식(`GameServer.exe`·`AccountServer.exe`·`BatchServer.exe`)을 다시 띄워 "0건" 확인 직후에 서버가 되살아난다.
    ```powershell
    Get-Process -Name GameServer,AccountServer,BatchServer -ErrorAction SilentlyContinue | Stop-Process -Force
...
- **밸런스 리밸런싱은 시뮬레이터로 측정해서 한다**: 스테이지 난이도·몬스터 스탯·직업 스탯·성장 곡선을 조정할 때는 `.claude/skills/rebalance` 스킬을 따른다. 손계산으로 수치를 정하지 말고 **`tools/balance_sim.py`로 측정 → 조정 → 재측정**하며, 정본 두 곳(`master-data-값.md`·`master-data-schema.sql`)을 함께 고치고 `master_monster_tool.py verify`가 오류 0인지 확인한다. **전투는 클라이언트 권위이므로 `python tools/master_data_export.py`로 클라 JSON 번들을 재생성하는 단계까지가 한 작업이다** — 정본만 고친 상태는 게임 안 수치가 그대로다. MySQL이 없어 번들 갱신을 못 하면 완료로 보고하지 말고 남은 단계를 알린다.
