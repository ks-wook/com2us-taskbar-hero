#!/usr/bin/env python
"""로컬 개발 환경 부트스트랩(**도커 경로**) — 컨테이너 기동부터 보스러시 랭킹 캐시 최초 적재까지.

실행 방법이 둘이다 — 이 스크립트는 그중 **전부 컨테이너로** 띄우는 쪽이다.
    server_up_with_docker.py (이 파일)  MySQL·Redis·서버 2개(+로그 파이프라인)를 도커로 띄운다.
                                        준비물이 도커뿐이라 새 PC의 첫 기동에 쓴다.
    server_up.py                        MySQL·Redis가 이 PC에 설치되어 있다는 전제로 **서버 2개만**
                                        dotnet watch(핫 리로드)로 띄운다. 서버 코드를 고치며 개발할 때.
...
사용법 (저장소 루트에서 실행한다 — docker compose가 이 위치를 기준으로 돈다)
    ...
    python server_up_with_docker.py --warmup-only # 이미 떠 있는 GameServer에 랭킹 캐시 적재만 지시
    python server_up_with_docker.py --down        # 그 묶음을 중지(볼륨·데이터는 남는다)
...
띄우는 것 — 둘 중 하나다(대화형 메뉴의 1번·2번이 각각 이것이다)
    ...
    두 묶음 모두 서버를 컨테이너로 띄운다. 서버를 콘솔(핫 리로드)로 띄우려면 `python server_up.py`를
    쓴다 — 의존 서비스가 컨테이너여도 된다:  python server_up.py --mysql-port 33306 --redis 127.0.0.1:36379
    (의존 서비스만 따로 올리려면 docker compose up -d mysql redis. 랭킹 캐시 적재는 그 스크립트가 한다)
...
def verify_schema() -> bool:
    ...
    if missing:
        ...
        say("  docker compose down -v  &&  python server_up_with_docker.py", "warn")
        return False
...
def main() -> int:
    ...
    print()
    if not warmup:
        say("랭킹 캐시 적재를 건너뜁니다(필요해지면: python server_up_with_docker.py --warmup-only).", "dim")
