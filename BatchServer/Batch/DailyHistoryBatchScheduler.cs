...
public sealed class DailyHistoryBatchScheduler : PeriodicBatchScheduler
{
    ...
    protected override TimeSpan MaxDelay => Constants.Batch.DailyHistory.MaxDelay;

    /// <summary>
    /// 다음 실행 시각(KST <c>RunHourKst</c>시 정각)까지의 대기 시간. 이미 그 시각을 지났으면 다음 날로 넘긴다.
    /// <para>고정 24시간 간격으로 두지 않는 이유는, 그러면 <b>실행 시각이 서버 기동 시각에 끌려다녀</b>
    ...
    protected override TimeSpan NextDelay()
    {
        ...
        var next = new DateTimeOffset(
            kstNow.Year, kstNow.Month, kstNow.Day, _runHourKst, 0, 0, DateTimeUtil.KstOffset);
        if (next <= kstNow)
        {
            next = next.AddDays(1);
        }
    ...
    /// <para>기동 직후에도 1회 돈다(골격의 기본 동작). 적재 테이블이 <c>(log_date, …)</c> 자연 키 PK라
    /// 같은 날 다시 돌면 <b>덮어쓰는 것이 정상</b>이고(7장), 예정 시각에 서버가 내려가 있었더라도
    /// 그날의 스냅샷이 비지 않는다.</para>
    /// </summary>
