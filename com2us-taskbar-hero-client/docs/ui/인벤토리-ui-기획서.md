...
- **범위 경계**:
  - 본 문서는 **인벤토리 보기 + 장착/해제 UI**를 1차 범위로 한다. 큐브(합성/분해/제작)·**장비 강화**는 별도 패널(`CubePanel`)이 담당하며([클라이언트 씬 구성](../클라이언트-씬-UI-구성.md) 3.4 기능 패널 표), 인벤토리 패널은 그 결과인 **강화 단계만 표시**한다 — 슬롯 좌상단 "+N" 배지, 아이템 이름 "+N", 옵션 스탯·능력치 합산에 `enhance_master` 배율 반영. 랜덤 상자·드래그 이동은 여전히 후속(10장)이다.
...
- 패널은 두 영역으로 나뉜다.
  ...
  - **능력치 패널**(초상 좌측): 최종 능력치 = **(클래스+레벨 기본 + 장착 장비) × 학습 패시브 배율**. 아이템·패시브로 인한 **상승분은 `(+상승분)`** 으로 병기하며, 그 텍스트만 **연한 파란색**(`#8FC1FF`)으로 표시한다(기본값은 흰색). 패시브 배율·statType 매핑은 전투(`PlayerCombatant`)와 동일하다.
