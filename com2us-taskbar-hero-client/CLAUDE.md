...
- **캐릭터 프리팹 애니메이션은 `AnimDevScene`에서 확인한다.** 씬은 에디터 메뉴 **`TaskbarHero/UI/애니메이션 개발 씬(AnimDevScene) 생성`**(`Assets/Editor/AnimDevSceneBuilder.cs`)으로 만들며, 구성은 카메라 · EventSystem · `Knight_Male` 프리팹 1개 · `AnimDevController`뿐이다(배경·조명 없음). 왼쪽 목록에서 애니메이션을 고르고 **캐릭터를 클릭하면 재생**된다 — 목록은 `SPUM_Prefabs`의 상태별 클립 리스트(IDLE·MOVE·ATTACK·DAMAGED·DEBUFF·DEATH·OTHER)와 `SpumCharacterAnimator`의 특수 모션(분노·돌진·캐스트 홀드·화살비·내려찍기)을 런타임에 읽어 만든다. `AnimDevController`는 `SPUM_Prefabs`가 있는 **Assembly-CSharp(`Assets/Scripts/Spum/`)** 에 둬야 클립 목록을 직접 열거할 수 있다(asmdef 어셈블리에서는 SendMessage만 가능). 빌드에는 포함하지 않는다.
  - **상단 중앙 패널에서 직업(기사·궁수·마법사·슬레이어)과 성별(남·여)을 골라 캐릭터를 교체한다.** 프리팹은 `CharacterPrefabDatabase.PrefabOf(classCode, gender)`로 찾아 같은 자리에 다시 만들고, 애니메이션 목록·무기 이펙트·카메라 프레이밍을 새 프리팹 기준으로 재구성한다(캔버스를 통째로 다시 만든다). 강화 단계는 유지되므로 직업만 바꿔 가며 같은 단계를 비교할 수 있고, 잔상 형태(`weaponTrailStyle`)는 직업에 맞춰 자동 전환된다(검·도끼 Melee / 활 Bow / 지팡이 Staff). 캐릭터는 `characterScreenX = 0.5`로 **화면 정중앙**에 온다.
  - **오른쪽 패널에서 무기 강화 단계(0~10)별 이펙트를 확인한다.** 단계 버튼을 누르면 `WeaponAfterimage`의 잔상 색·무기 틴트·불꽃 입자가 즉시 바뀐다. 궤적(잔상) 방출 조건은 **전투와 동일하게 공격 모션 재생 여부**를 쓰므로(`AnimDevController.IsAttackMotionPlaying`), ATTACK 클립을 재생할 때만 무기 끝에 궤적이 생기고 IDLE에서 보이는 것이 곧 **상시 이펙트**(무기 틴트 + 불꽃)다. 잔상 형태(`weaponTrailStyle`)와 시작 단계(`startEnhanceLevel`)는 씬 인스펙터에서 바꾼다. Assembly-CSharp는 `autoReferenced` asmdef를 자동 참조하므로 `TaskbarHero.Client.Battle`을 그대로 쓸 수 있다.
...
- **아이템이 놓이는 칸은 공용 슬롯 프리팹 하나만 쓴다 — 화면마다 새로 그리지 말 것.** 아이템 아이콘을 보여 주는 모든 UI(인벤토리 가방 칸·장비 부위 칸 · 큐브 4탭 타일 · 거래소 · 우편함 첨부 · 출석부 · 뽑기 결과 · 스테이지 클리어 보상)는 **`Assets/Prefabs/UI/ItemSlot.prefab`(`ItemSlotView`)** 을 인스턴스화해 소비한다. 슬롯이 책임지는 표현은 **등급 배경 · 아이콘 · 우하단 수량 "xN" · 좌측 하단 "+N" 강화 배지(흰 글자 + 검은 외곽선) · hover 상세 팝업**이며, 이 규격은 `ItemSlotView`의 상수(`EnhanceBadgeColor`·`EnhanceBadge*`)로 고정되어 있으니 **화면별로 색·자리·크기를 다시 정하지 않는다**(2026-08-05에 인벤토리 좌하단 금색 32px vs 큐브 우상단 금색 20px으로 갈라져 있던 것을 통일했다).
  - 화면 고유 표시는 슬롯 **밖/위에** 얹는다 — 선택 프레임, 아이템 이름, 태그(레시피 `Lv.N`·`장착`), 착용 불가 X, 드래그 입력.
  - 칸에 이미 자기 프레임(`ui_slot_normal` 등)이 있으면 `SetFrameVisible(false)`로 슬롯 프레임을 감춰 테두리가 겹치지 않게 한다. 타일 전체가 버튼인 화면은 `Setup(..., showDetail: false)`로 슬롯이 클릭을 가로채지 않게 한다.
  - 새 화면을 추가하면 컨트롤러에 `[SerializeField] GameObject _itemSlotPrefab`을 두고 **`Assets/Editor/ItemSlotBuilder.cs`의 배선 목록**(`WireSlotPrefabField<T>`)에 한 줄 추가한다. 배선 실행은 메뉴 `TaskbarHero/UI/아이템 슬롯·상세 팝업 배선`.
  - **강화 배지에 `resizeTextForBestFit`을 쓰지 말 것.** 크기는 `EnhanceBadgeFontHeightRatio`(칸 높이 × 0.352)로 직접 넣고 overflow는 가로·세로 모두 `Overflow`로 둔다. BestFit을 쓰면 `UiTextStyle`이 0.4초 주기 스캔에서 `verticalOverflow`를 Truncate → Overflow로 바꾸는 순간 크기가 재계산되어 **글자가 작아졌다 커지는 현상**이 생긴다(큐브에서 타일을 누르면 그리드를 다시 만들어 배지가 새로 생성되므로 클릭마다 재현됐다 — 2026-08-05 수정). 칸 크기가 정해지기 전(높이 &lt; 24px)에는 배지를 그리지 않고, `OnRectTransformDimensionsChange`에서 제 크기로 한 번에 띄운다.
...
- **접속 서버 주소는 `Assets/Scripts/Managers/ServerEnvironment.cs`에만 정의한다.** 빌드 옵션은 **Dev**(로컬 `http://localhost:5160`·`5247`)와 **QA**(`https://ksu864-1.tail961c4e.ts.net`·`:8443`) 둘이고 **차이는 접속 서버 주소뿐**이다. 주소를 씬·프리팹·다른 스크립트에 하드코딩하지 말 것(`NetworkManager`의 인스펙터 문자열도 표시용이며 `Awake`에서 이 프리셋으로 덮어쓴다). QA 여부는 스크립팅 정의 심볼 `TH_QA`로 판정하며, `FullGameBuilder`의 QA 메뉴가 `extraScriptingDefines`로만 넣으므로 **Player Settings·에디터 컴파일 상태를 건드리지 않는다**. 환경 선택은 **PlayerPrefs에 저장하지 않는다**(Dev/QA 빌드가 같은 product로 PlayerPrefs를 공유해, 저장하면 QA 빌드가 로컬로 붙는 사고가 난다). **QA 빌드는 접속처가 원격으로 고정**이라 서버 선택 화면(`ServerSelectPanelController`)이 뜨지 않고 저장된 호스트 override도 무시한다(판정: `ServerEnvironment.AllowServerSelection`) — 선택 UI는 Dev 빌드·에디터 플레이 전용이다. 상세: [`docs/빌드-옵션.md`](docs/빌드-옵션.md).
...
- **커밋은 사용자만 한다 (예외 없음).** 어시스턴트는 `git commit`·`git push`를 **어떤 상황에서도 스스로 실행하지 않는다.** 작업이 끝나면 **변경 사항 요약만 전달하고 멈춘다** — "커밋할까요?"는 물론 **"커밋하지 않았습니다"·"별도 커밋으로 나누는 게 좋겠습니다" 같은 상태 보고·단위 제안도 붙이지 않는다.** 사용자가 **명시적으로 커밋을 요청했을 때만** 커밋한다. (상위 저장소 `CLAUDE.md`와 같은 규칙 — 클라·서버 어느 세션에서도 동일하다.)
