namespace TaskbarHero.Client.Managers
{
    ...
    /// 실행 중에는 접속 서버 선택 화면에서 바꿀 수 있으나 그 선택은 <b>해당 실행에만</b> 적용된다(저장하지 않는다) —
    /// PlayerPrefs는 Dev/QA 빌드가 같은 product 이름으로 공유하므로, 저장하면 Dev에서 고른 값이 QA 빌드의
    /// 기본 접속처를 덮어써 "QA 빌드인데 로컬로 붙는" 사고가 난다.
    /// </para>
