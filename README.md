...
| 9 출석부 보상 | Game | ☑ | ☑ |
| 10 거래소 / 교역선 | Game | ☑ | ☑ |
| 11 인벤토리 조회 페이징 처리 | Game | ☑ | ☑ |
| 12 캐릭터 성별(남/여) 선택(`player_character.gender` · 생성 시 확정 · 외형 전용) | Game | ☑ | ☑ |
| 13 가챠(뽑기) 시스템 구현 ([기획서](docs/세부/gacha-기획서.md)) | Game | ☑ | ☑ |
| 14 장비 강화 시스템(`inventory/enhance` · `enhance_master` +10 · 확정 상승) | Game | ☑ | ☑ |
| 15 소모품 사용·활성 버프 조회(경험치·골드 부스터 · [기획서](docs/세부/consumable-buff-기획서.md)) | Game | ☑ | ☑ |
| 16 캐릭터 생성 시 직업 기본 무기 지급·장착(`item_master` 최저 등급 무기 · 생성 트랜잭션 동시 적재 · 클라는 기존 `load`의 `equipped[]` 경로 그대로 사용) | Game | ☑ | ☑ |

> ☐ 미착수 · ◐ 진행 중 · ☑ 완료 · ✕ 해당 없음(작업 불필요) — 진행 시 해당 칸 기호를 바꿔 표시한다.

### 추후 해볼만한 것

> 지금 범위에는 넣지 않았지만, 이어서 한다면 우선 볼 후보다.

| 항목 | 내용 |
|---|---|
| 운영툴 개발 | 계정 · 재화 · 아이템 조회와 지급, 메일 발송, 시즌 · 마스터 데이터 확인을 화면에서 하는 내부 도구. 지금은 DB에 직접 붙어야 한다 |
| 로그 기간별 관리 기능 추가 | logdb에 쌓인 로그를 기간 단위로 조회 · 정리(보존 · 삭제 · 내보내기)하는 기능. 지금은 액션 로그 90일 보존 정책만 있고, 서버가 쓰는 이벤트 로그 파일은 정리 주체가 없다 |
